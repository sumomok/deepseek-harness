...
帮助 → 检查更新 位置与文案都不变,因为一个消失了的菜单项没人再找得回来。它跑的是同一个静默检查,只答「已是最新版本」或「无法检查更新」:点击值得一个回复,而这两句就是那个回复。查出活儿来的检查,答案在更新所在的地方——因为一个写着「正在下载」的对话框,正是这次改动要拿掉的那种打断。

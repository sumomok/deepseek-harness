...
Every step but `wait` names its element twice: by the ref a read returned, and by the name that read printed. The seat resolves the ref, computes the name again with the reader's own naming — one exported function, which the pass over the page calls for every row it prints and the seat calls for the one element a step names — and refuses the step when they differ. Two computations of it would be worse than none: a page whose listing prints a name the check cannot reproduce is a page every step naming that element is refused on, which is what a console's own query box was until the two were made one. This is the failure the whole design is arranged around: a page that re-rendered its table between the read and the call has the same refs pointing at different rows, and a step that trusted the ref alone would press whatever now sits there — the one mistake a later read cannot undo. Four more endings stop a step: an element the page no longer has, one it no longer shows, one behind a dialog the page has put in front of the user, and one the page has switched off. The visibility check is the reader's own, walked over the ancestors, and it runs before the name: a hidden element still answers to its name, still takes an event and still runs the handler behind it, so nothing else on the way to a step would stop one — and what the model would be doing is pressing a control the user cannot see.

The check is not an identity. Two rows whose buttons are both called 编辑 pass it, and a page that renumbers and renames together is a page a read has to be taken of again. It is what catches the common case — the page moved — at the cost of one naming per step.
...
**This gate is retired**; [the sign-in pages note](2026-09-04-sign-in-pages-are-read.md) supersedes it, and a page showing a sign-in form is acted on like any other. The paragraph below records what the gate was.

`content_read` refused to hand over the listing of a page showing a sign-in form. A channel that types into one would be the way around that rule, so the same verdict — the reader's own, taken from the page's header — gated the steps: a call against such a page failed before its first step and said so. The closing read was checked again, because the steps themselves can produce one (a sign-out, a session that expired mid-call), and its structure was withheld from the report the way the read withheld it. The refusal was a wire code of its own, worded by the seat.
...
### What the browser did on the page's own account comes back with the answer

A step is one event dispatched at one element; everything the application does in answer to it happens afterwards. A `confirm()` nobody answered would block the frame's event loop until the deadline, and a window opened behind the console is one nobody will look at. For the length of the call — and no longer — the seat listens for route changes and compares the address, and stands in for `confirm`, `alert`, `prompt` and `window.open`. A link that would open a new window is stopped and reported the same way.

**The message watch is retired** — the two paragraphs below record what it was and what its removal gives up; [the general-tools note](../simplification/2026-09-03-content-reader-general-tools-only.md) owns that decision. What the page draws in answer to a step is in the closing read and in no line of the report.

What makes a message is the moment the text becomes readable, not the moment it was written. An application writes its toast into a box it keeps hidden and then shows the box, so watching for added text alone sees the words on no screen and reports nothing — which is what a recorded run found. The watch follows nodes rather than text: what a mutation touched — a node added, text rewritten in place, an element whose `hidden`, `aria-hidden`, `class` or `style` changed — is followed from that point, and what it shows is compared with what it showed last. `hidden` and `aria-hidden` say for themselves whether they were hiding it; a class or an inline style cannot be read back once changed, so an element that shows text after one is taken to have just been given it — a page restyling something it was already showing spends one line, and the other way round would miss every framework's own reveal.

A message is reported when the page takes it away, with how long it stayed, and at the end of the call when it has not: the closing read shows what is still on the page and never says the steps produced it. Both halves are bounded — eight lines a call, thirty-two nodes followed at once, and no work at all once the lines are spent — and the lines a vanished message takes are pushed as it happens, so a page that churns cannot crowd them out with what is still on the screen.
...
The report route's envelope is the read's own. Every report spends two of the envelope's four names on the call's id and the tab's; a report of steps spends the other two on the page's id and title, and adds the document's title, its body against the budget, and one step list — spending nothing on the address, the other header field, or the cursor, which leaves 9,120 bytes of the allowance unspent. Inside that, the step list costs the failing step's message, which is the same 2000 characters the failure arm is bounded by and which the parser holds to one step, plus about 35 bytes of punctuation per step. `MAX_ACT_STEPS = 100` is what keeps that punctuation inside the rest, and `maxSteps` is refused above it at load.
...
**Trusting the ref alone.** Simpler, faster, and wrong on exactly the pages this is for: a console table that re-renders on a filter keeps its refs pointing at rows that moved. The name check costs one naming per step and turns a wrong click into a refusal.

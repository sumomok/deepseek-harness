...
**在客户端把这一行过滤掉。** 没有缝。`CommandUiContract` 只能增：`register` 添加一个客户端行，且在与宿主命令重名时抛错（`packages/client/ui-commands/src/client/service.ts:260-262`），`decorate` 明确既不能凭空造出一行也不能移除一行（`src/client/contract.ts:69-72`），`candidates()` 没有任何钩子，宿主的 `list()` 则是一个返回冻结有序数组的普通 `@Remote` 方法——没有 waterfall，没有否决权，`CommandDescriptor` 也不携带可见性位。
...
`packages/experimental/server-sidebar/tests/customer-overlay.client.spec.ts` 用 `js-yaml` 与 loader 的 entry schema 解析出厂 overlay（沿用 `packages/experimental/content-frame/tests/permission-gateway-overlay.client.spec.ts` 立下的做法），钉住原先由 e2e 守着的那个闭集：表序下的三个预设 id、它们面向客户的名字、`isolate.commands === true`、一个点名其中之一的 `defaultPreset`，以及 `ui-permission` 禁用行——然后断言 `apps/web/tests/` 下那三份 overlay 携带的这两行完全一致。
...
`examples/content-console` 是控制台的免钥快照泳道，这里不需要它做任何事：它只组合后端骨干，从不应用 `overlay/customer.patch.yml`，也根本没有组合 `permission-presets`——它自己的模块文档记着，浏览器那些行的证据在 `apps/web/tests` 下的 Playwright 泳道。本次改动只落在那份 overlay 与浏览器可见的界面上，因此不会挪动任何 transcript。

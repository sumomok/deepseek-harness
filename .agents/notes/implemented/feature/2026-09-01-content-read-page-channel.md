...
The seat measures that block against both bounds, because the character one is not the tighter of the two. The route takes a body of the budget at four bytes a character plus the envelope, and a page written in three-byte characters spends that allowance at three times the rate its character count suggests: at the shipped budget of 12,000, an eighty-column table renders to 32,696 characters — well inside the character bound of 48,000 — in a report of 97,097 bytes against a bound of 71,328. What the seat measures is that report. It builds the document it is about to post and serializes it the way `JSON.stringify` will, escapes included, so it counts the bytes the route counts rather than estimating them, and the call id — the one field no bound holds before the parser has read it — is on the scale at its own size. Both halves have work. The envelope is 23,328 bytes of the route's allowance, so the byte bound sits above four times the budget even in one-byte text: 119 columns of ASCII render to 48,375 characters in a body of 49,114 bytes, which the character half refuses and the byte half would have taken. It is also why the floor's promise holds for a page in any language: eight columns of three-byte text is 3751 characters in a body of 11,128 bytes against 27,328.

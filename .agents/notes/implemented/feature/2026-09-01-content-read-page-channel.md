...
The seat measures that block against both bounds, because the character one is not the tighter of the two. The route takes a body of the budget at four bytes a character plus the envelope, and a page written in three-byte characters spends that allowance at three times the rate its character count suggests: at the shipped budget of 12,000, an eighty-column table renders to 32,696 characters — well inside the character bound of 48,000 — and to 96,845 bytes against a byte allowance of 48,000. The seat measures the listing as `JSON.stringify` will write it, escapes included, so it counts the bytes the route counts rather than estimating them; measuring the listing alone covers the whole body, because the envelope already allows every other field its own character bound at those same four bytes and each of them passes through the seat's removal and clipping first.

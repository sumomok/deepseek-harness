...
- The prompt still has to ask for JSON. The endpoint requires it: without it the model can stream whitespace until the output cap.
...
The field is deliberately absent from `LlmCallConfig`. The agent loop spreads `header.config` into every loop-built request, so a conversation request cannot acquire an answer format, and the logged request header keeps reconstructing the exact request the model saw. The agent-loop request-reconstruction invariant enumerates that equation field by field and now requires `responseFormat === undefined`, so the absence is checked rather than merely arranged: a loop request carrying an answer format fails with `diverges from the folded request header`. Only a direct `ctx.llm.stream()` caller — the judge, like the title and compaction providers before it — can set it, and those auxiliary calls already sit outside the loop's logged header.
...
**Strengthen the judge's prompt instead of the request.** Rejected as insufficient rather than wrong: the gateway's prompt already asks for JSON, and JSON mode requires that it keep doing so. The prompt already asks, and asking is what failed.
...
This is a fork overlay on an upstream core package. It retires when upstream's `GenerateOptions` gains `responseFormat` or an equivalent answer-format field, at which point the fork's gateway adapts to upstream's form. The check reads the declaration site alone — `git grep -n "responseFormat" upstream/master -- packages/llm/llm/src/types.ts`, zero hits today — because the whole `packages/llm` path already matches three `supported_parameters` entries in a pi-ai model-listing fixture. A non-zero result is a signal to read, not a verdict. Until then it is re-ported on every rolling sync, because it lands in the type upstream edits whenever the request field set grows.

A producer that sets the field owes its routes a fallback. `LlmRuntime` turns an adapter throw into a terminal `error` finish, so on a pi-ai route — selectable for any model in the Models page — a judge that sets `responseFormat` gets `UNSUPPORTED_OPTION` on every call and fails closed on every tool call, which is worse than the single prose answer this field removes. The obligation belongs to the producer: recognize that code, retry the same request once without the field, and cache the answer per route. The gateway carries it from 0.4.6, which the desktop payload vendors.

No `packages/bundle/*` profile sets the field — the producer is a vendored plugin the desktop payload mounts — so no session log, snapshot, or recorded fixture changes: an absent `responseFormat` serializes byte-for-byte as before. The evidence is in the three packages that read it — `llm-deepseek`'s `serialize.spec.ts` pins the mapped wire field and its absence when the request names no format while its `adapter.spec.ts` pins the same pair on the body a `ctx.llm.stream()` call actually puts on the wire, `llm-pi-ai`'s `adapter.spec.ts` pins the `UNSUPPORTED_OPTION` failure before any request, and `agent-loop`'s `invariant.spec.ts` pins the loop-request rejection.

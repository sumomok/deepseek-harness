...
`conversation.chat.stats.usageLabel` is a single slot taking the leading segment of the usage pill's label, where the shipped pill prints its abbreviated token total. The pill keeps everything after it — the separator and the cache-hit segment still render — and the seat's fallback is that same token total, so an unoccupied pill is the shipped one. The fallback covers an empty seat, not a broken occupant: a registered component that throws is caught by the entry's error boundary, which renders `<div data-slot-error>` in its place, so the pill shows no token total at all. An occupant therefore has to carry its own failure reading.

`conversation.chat.stats.usageRows` is a list slot rendered inside the dialog's `dl[data-session-stats-usage]`, after the output row. An entry emits its own `dt`/`dd` pairs and inherits the list's skin, with one structural caveat: every `renderSlot` site renders through `SlotOutlet`, which always wraps its content in the framework's `<div data-slot="<key>" style="display: contents">` anchor. The pairs are therefore grandchildren of the `<dl>`, not children. Both the grid and the skin still resolve — `display: contents` promotes the pairs to grid items of `.details`, and `.details dt` / `.details dd` are descendant selectors — and the anchor stays out of the accessibility tree. The accepted cost is that the `<dl>` now has a `div` among its element children, which HTML's content model for `dl` does not admit; rendering and assistive technology are unaffected, and the same anchor-inside-phrasing-content pattern already ships elsewhere (`sidebar.brand.mark` inside a `span`). With no entries the dialog is the shipped one.

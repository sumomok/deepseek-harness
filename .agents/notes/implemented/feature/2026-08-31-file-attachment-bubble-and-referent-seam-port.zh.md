...
## 后续补丁：为 dsh-client-bundle-purity 把 dispatchReferentOpen 服务化（B.5 收尾）

上面第二段那句「不受 `verify-client-packages` 限制」并不完整——这一点直到本次同步收尾阶段第一次跑全量 `pnpm run build`（自本补丁落地以来的第一次）才发现。`verify-client-packages.ts` 的静态 AST 检查与 `packages/client/tsdown.client.ts` 的 `dsh-client-bundle-purity` tsdown 插件，是为同一条设计意图各自独立实现的两道门禁（`packages/client/*` 特性包跨插件协作只允许走类型或调用一个注入的 Cordis 服务，绝不允许裸函数跨插件导入）——`verify-client-packages.ts` 从未建模插件闸门实际执行的这条规则，因此 `apply.ts` 对 `dispatchReferentOpen` 的值导入自本补丁落地那一刻起，每次静态检查都误报清洁，同时却一直真实违反捆包纯度。只有对 `ui-chat/client` 的真实 tsdown 构建才会触发第二道闸门，而本次同步直到收尾阶段之前从未跑过一次。

**`dispatchReferentOpen` 现在经一个新的 `ctx.referent` 服务（`referent.ts` 自己文件里新增的 `ClientReferent extends Service`）触达 `packages/client/*` 消费方，而不是靠 `dsh.client.external` 声明。** 给 `ui-chat/package.json` 的 `dsh.client.external` 加一条 `@deepseek-ai/dsh-api-session-controller/client` 是最初尝试的修法，但这条路对这一类别的包是死路：`verify-client-packages.ts` 对任何 `packages/client/*` 清单声明的、指向另一行 specifier 的 `external` 都无条件拒收（报错原文本身就点名了合规出路——"import shared types only or call an injected Cordis service"）；最初援引的两个先例——`packages/api/session-controller/package.json` 与 `packages/api/workspace-controller/package.json`——虽然也声明了 `external`，但两者都不在 `packages/client/*` 之下，因此从未落进这一条检查分支。没有任何 `packages/client/*` 行声明 `external` 指向另一行的现成先例；自由函数改服务是同时满足两道门禁的唯一路径。`ClientReferent` 的 `open(ref, onDefault)` 只是一行转发给 `dispatchReferentOpen(this.ctx, ref, onDefault)`——行为完全一致，因为 cordis 会把一个 `Service` 子类的 `this.ctx` 重新绑定到调用方实际持有的 context（与 `ctx.conversation`／`ctx.workspaces` 已经依赖的机制相同），因此 `ctx.referent.open(...)` 派发时用的正是调用方所在插件自己的 context，与原本直接调用自由函数逐字节一致。`dispatchReferentOpen` 的自由函数导出、以及 `referent.ts` 里的其余全部导出都未改动——仓库之外的 `clickable-refs` 插件仍经自己的 shims 直接消费自由函数，这次同步不动它（它下一次重新 vendor 时再自行决定是否改走服务，还是自己声明一条 external——它根本不是受这条规则约束的 `packages/client/*` 行）。

**这与上文 `ctx.workspaces.openPath` → `ctx.remote.session.openWorkspacePath` 那条落点决策同属一类适配，不是新引入的设计债务。** 原始补丁的自由函数形态在重组前的旧 `client/runtime` 包自己（更宽松）的规则下是成立的；落进 `ui-chat` 这一新的 `packages/client/*` 类目，就必须按这一类目真实的约定去适配——与本次移植的其他决策把调用点适配到重组后代码树的真实服务面、而不是照搬旧基座恰好允许的形态，是同一件事。

`packages/test-support/client-runtime` 的 `SlotTestRuntime` 构造函数现在也挂载一个真实的 `new ClientReferent(ctx)`，与既有的 `sessions`／`workspaces` 测试替身并列——用的是真实实现而非手搭桩件，因为它是一个无状态的薄封装，每个测试台因此免费获得与生产环境一致的真实 `referent/open` waterfall 行为。此前受影响的全部 142 个测试文件（2052 条测试）除了运行时构造函数那一行改动之外零改动直接通过，证实这层服务封装在行为上完全透明。`referent.client.spec.ts` 新增了专门的 `describe('ClientReferent (ctx.referent service)', ...)` 代码块：注册、`open()` 与自由函数一致的默认动作/认领委派语义、这整套设计所依赖的跨作用域 `this.ctx` 重绑定、以及 fiber 卸载时的自动退订。`scripts/gen-cordis-catalog.ts` 的 `SERVICE_WALK_EXEMPTIONS` 新增了 `referent` 一条（记述在本包自己的 README、紧挨 `dispatchReferentOpen` 既有的那段文字旁）——`IReferent` 是与 `chatFileMentions`／`proseReferents` 同类的小型访问器式服务，不像 `sessions` 那样需要一条渲染进 `SERVICE_PAGE` 的目录条目。

...
**Three tiers for the route.** `effectiveRoute` reads a `model/selection` no request has consumed, then the session's logged `request/header`, then the agent's options. The first two, and their order, are [`selectionFor`](../../../../packages/api/session-controller/src/agent.ts)'s own; the third is not. The controller falls back to the deployment default, which this package's Client-face program cannot read without a further dependency, and nothing reaches that tier inside a tool execution — a tool call implies a request and a request logs a header — so the divergence is recorded rather than closed. An absent `inputModalities` is a negative answer at every tier.

**One decision at a time per session, and one card per session.** Route decisions for one session are serialized through a `WeakMap`, so the second of two parallel reads runs after the first has changed the model and reads that change as its own first tier — it passes without a second card. A user who answers with anything other than a route is recorded in a `WeakSet`, and that session's later reads refuse without asking again; the mark suppresses the card and never a pass. Only a decision is recorded: a card the user closed, and one raised for a delegated caller that can never reach a human, settle the question. Every other rejection leaves no mark, so the next read asks again — a caller the registry no longer holds, a transport that dropped while the card stood, an answerer that threw, and `NO_PROVIDER`, which says that no console was connected at that moment rather than that anyone declined; a console opened a minute later must still be asked. The two codes that count (`ASK_CANCELLED`, `DELEGATED_CALLER`) are read off the rejection rather than off a class, because it reaches this package restored from a remote waterfall.
...
**The card lists what the deployment claims.** `imageCapableRoutes` walks `ctx.llm.listProviders()` and `listModels(provider)`; the `modelCatalog` projection the console's picker renders carries no modality and cannot answer this. A provider whose catalogue throws is left out and the rest of the card stands, and a composition with nobody to ask refuses before any provider is walked. Catalogue membership is advisory, and `resolveCallConfig` inside `selectModel` checks that the chosen route resolves rather than what it accepts — its refusal reaches the model as `routeSwitchRefusal` — so the route the session moved to is put through this gate's own criterion before the read runs. A deployment whose catalogue and whose resolved route disagree ends in the refusal a text-only route earns, with the change made and no picture exported.
...
- **1 — new surfaces: 2.** `imageCapableRoutes` and `optionLabels`. Both run after the gate has read the two services a card needs, so a composition that cannot ask never walks a provider.
...
| Assumption | `listModels` is what the deployment claims and nothing more. `selectModel` checks that the chosen route resolves, not what it accepts, so the modality criterion is applied a second time to the route the session moved to. | A card offering a route that cannot be switched to, with nobody saying why; and a catalogue that claims `image` for a route the adapter resolves without it, which would store a picture the request drops. | permanent |

#### M5 — the first tier, the decision chain, and the decline mark

- **0 — what a settled principle already answered.** "Prefer symmetry for parallel values; unexplained asymmetry usually signals a missed extraction" is what made the missing tier a defect rather than a choice — `selectionFor` reads a pending selection first and this read did not, for no stated reason. The same rule leaves the third tier asymmetric on purpose: reading the deployment default would cost a dependency this face cannot take, and no tool execution reaches it. "Runtime invariants assert owned relationships" kept the chain and the mark out of the log and out of every projection: they are this package's own runtime state. The defensive-patterns rules on lifecycle put both in weak collections keyed by `Session`.
...
| Red line | The mark suppresses the card and never a pass, and only a decision sets it: a closed card and a caller that can never be asked settle the question; a console nobody had open and a broken channel do not. | A user who changed the model in the console still being refused; and a console closed at that moment, or a browser tab reloaded while the card stood, costing that session the offer for the rest of the process. | permanent |
...
**A card that reached nobody is not an answer.** The one-card mark records a decision only, so a console nobody had open, a dropped connection, or a failing answerer leaves the offer standing for the model's next attempt, at the cost of another card.
...
One Web scenario backs it: `apps/web/tests/content-read-image-switch.e2e.ts`, over the same composition, application and preset as [the picture scenario](2026-09-04-content-read-image.md), and differing in one thing — it does not select a route on the seeded session, so the session stays on the `deepseek-v4-flash` its seed logged. The spec asserts what the two routes declare before it drives anything, waits for the card, and asserts the route there rather than up front: a session nothing has opened is not live host-side — the other content scenarios are live only because selecting their route resolved their agent — and while the card stands, the request that reached the gate is already logged, so what is asserted is that that request ran on `deepseek-v4-flash` and that no selection was pending. It then pins the card's accessibility snapshot as `card.expected.md`, clicks the vision option and submits, and asserts that the answer describes the picture, that a `model/selection` was appended, and that a `reason: 'change'` request header on the vision model followed it. The click is the test's own action in record and replay alike, the way the approval scenario's is. The scenario's request headers differ from the picture scenario's from the first one, so it carries a `header.class` and a pin of its own and grows two sidecars with a `<!-- request/header change 1 -->` section in them. Record it with a key:

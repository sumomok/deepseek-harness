...
The fork lands the tone in two places, not one. `packages/bundle/base/cordis.patch.yml` marks `danger-full-access`, and the permission-gateway bundle's `cordis.patch.yml` marks the same row in the table it restates. An id-targeted patch replaces the `permission` entry's whole `config`, and `BUILTIN_WEB_BUNDLES` seeds that bundle into the desktop profile, so the gateway's copy is the one a desktop install runs: with only the base line, neither menu colors anything there. `apps/desktop-shell/tests/builtin-permission-gateway.spec.ts` is red until the gateway restatement carries the line, and the integration line closes it by vendoring `@haoran/dsh-llm-permission-gateway` 0.4.5 or later. The gateway's `yolo-access` stays untoned, as does the plugin's own default table, so a composition that does not ask for the tone renders exactly as before.
...
**Derive the tone from `glyph`.** Rejected: `yolo-access` names the full-access glyph precisely because it is a full-access bundle, and it is the row that must not be red. Glyph answers which shield to draw; tone answers whether the row is destructive.

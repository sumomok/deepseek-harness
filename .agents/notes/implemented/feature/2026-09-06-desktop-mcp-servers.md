...
**A stdio server runs outside the sandbox.** The MCP SDK starts the child process itself rather than through `ctx.shell`, so no sandbox mode the application offers reaches it and it runs with the account and permissions of whoever is using the computer. Saving the server is agreeing to that, which is why the form asks for the full path of the program it is about to start.

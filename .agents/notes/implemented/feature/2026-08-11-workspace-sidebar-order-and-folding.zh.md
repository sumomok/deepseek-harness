...
创建“新会话”并选中空白 Session 时，浏览器会在其分组记账和单列表记账中各置顶一次。这次明确的创建置顶不会推进 `updatedAt`。Session 保持空白期间，分组折叠不会让它占用五条非空白行配额；首条提示词或首条命令落地后，该行恢复普通配额，同时不会撤销手动模式下的拖拽。
...
- 新选中的空白“新会话”行会在分组和单列表顺序中各置顶一次。分组折叠会在五条非空白行之外额外渲染该行，直至首条提示词或首条命令落地后恢复普通配额。

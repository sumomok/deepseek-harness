# Agent Note: 把「密钥容器确认」补丁移植到 0.1.2 重组之上
...
**确认由 `intakeFiles` 触发；两个提交入口都不带闸门。** 按 `16daeaa345`：整批文件——命中与未命中一视同仁——先真实附着，弹窗随后只提供撤销命中子集的动作：「仍要添加」是纯关闭，「不添加」按命中 id 逐个调用 `removeImage`。命中文件的 id 无法同步得知，因为本代码树的 `addFiles`（`apply.ts`）丢弃了 `createDraftFiles` 的返回值、只回传拒绝原因字符串，因此 `intakeFiles` 先记下原始 `File` 引用，再由一个监听 `fileAttachments` 的 `useEffect` 在下一次渲染时把它们解析为 `DraftAttachmentId`。于是重组后代码树里那个仅以 `[editor, keyboard]` 为依赖数组注册的 Lexical 键盘映射与本能力无关：`gate.current` 保留既有的 `intakeDrop`，`submit:` 分支直接调用 `keyboard.submit(...)`，主发送按钮的 `onPrimary` 同样如此。

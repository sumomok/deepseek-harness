...
Member labels are the registrants' own copy, unchanged, so a group title and a member beneath it can read the same word: General and Models each repeat in English, 模型 repeats in Chinese (the General group's Chinese member reads 通用设置, which does not). The group title is the shell's copy (`nav.group.*`) and the row's is the registrant's (`general.nav`, `models.nav`). Renaming either would change shipped product copy for a cosmetic gain, and the row labels are not this change's to rename.

...
**Two id-targeted overrides, both complete values.** `agent-default-model` becomes `{provider: deepseek-official, model: deepseek-flash}` — DeepSeek V4.1 Flash, which upstream made `@deepseek-ai/dsh-base`'s own default in `bc5fd3b8dc` — so an Agent created without a session-local selection starts on a vision model. `llm-deepseek` gets a `models` catalog holding that model alone, named `DeepSeek-V4.1-Flash`: DeepSeek retired `deepseek-v4-flash` and `deepseek-v4-flash-vision-exp` and reroutes `deepseek-v4-pro` to V4.1 Flash from 2026-09-14, so every other row would offer a choice that is not one. A whole-table replacement is what allows both the drop and the addition, since the installed adapter still carries the four. The same popup carries a reasoning-effort level also named `Default` and the picker trigger spells a selection `<model> · <effort>`, so a session pinned to this row reads `default · Default`. The row's `id` is unchanged, so the relabel never reaches the wire.
...
What does change for such a user is the picker rows: `deepseek-flash` is the only one listed, whatever their stored selection is, because the catalog is a composition entry rather than a per-user one. A stored selection naming a retired model keeps working for text — DeepSeek serves those names with V4.1 Flash — but not for images: the adapter treats a model absent from the catalog as text-only, so a picture is refused until that person picks DeepSeek-V4.1-Flash. Their own `cordis.patch.yml` is untouched, since the seed never edits the user patch layer.
...
A fresh desktop install opens its first session on `deepseek-flash`, so image input works without anyone choosing a model, and the picker offers that model and no other. The version is the installer's, like every other built-in: changing it means shipping a desktop build.
...
Three fields are deliberately absent from the row — `contextWindow`, `imagePixelBudget`, and `imageMaxBytes` — because each falls back to the adapter value the factory rows themselves carry. Inheriting keeps this table from pinning numbers upstream may move; the drift test resolves each against the installed adapter and pins the row's stated fields outright, the adapter having no row of its own for this model.

...
**Two id-targeted overrides, both complete values.** `agent-default-model` becomes `{provider: deepseek-official, model: deepseek-flash}` — DeepSeek V4.1 Flash, which upstream made `@deepseek-ai/dsh-base`'s own default in `bc5fd3b8dc` — so an Agent created without a session-local selection starts on a vision model. `llm-deepseek` gets a `models` catalog that restates the factory rows verbatim and heads them with a `deepseek-flash` row labelled `default`; that row is the one with no factory counterpart, since a whole-table replacement is the only way a model the installed adapter does not carry reaches the picker. The same popup carries a reasoning-effort level also named `Default` and the picker trigger spells a selection `<model> · <effort>`, so a session pinned to this row reads `default · Default`. The row's `id` is unchanged, so the relabel never reaches the wire.
...
What does change for such a user is the picker rows: `deepseek-flash` is listed, and listed as `default`, whatever their stored selection is, because the catalog is a composition entry rather than a per-user one. Their own `cordis.patch.yml` is untouched, since the seed never edits the user patch layer.
...
A fresh desktop install opens its first session on `deepseek-flash`, so image input works without anyone choosing a model, and the picker names that row as the default it is. The version is the installer's, like every other built-in: changing it means shipping a desktop build.
...
Three fields are deliberately absent from each vision row — `contextWindow`, `imagePixelBudget`, and `imageMaxBytes` — because each falls back to the adapter value the factory rows themselves carry. Inheriting keeps this table from pinning numbers upstream may move; the drift test compares every inherited value against the installed adapter, and pins the added row's fields outright because it has no factory row to inherit from.

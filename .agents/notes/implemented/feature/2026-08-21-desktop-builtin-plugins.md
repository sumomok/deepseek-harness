...
**The closure.** `apps/desktop-server/package.json` declares `dsh-better-sidebar` and `dsh-at-file` as `file:` tarballs beside the workspace packages, so `pnpm deploy` materializes them into `resources/server/node_modules` with everything else the server closes over. The installer that carried a build owns those versions; there is no separate update channel for a plugin. `dsh-better-sidebar` has since been [withdrawn in 0.1.0-rc.33](../simplification/2026-09-14-desktop-withdraw-better-sidebar.md); the mechanism is unchanged and still carries `dsh-at-file` and the built-ins added since.

**The versions.** `dsh-better-sidebar`'s floor is `0.14.0`, and it is a floor rather than a preference: `0.1.0-rc.8` stopped exposing the `window.__DSH_MODULES__` page global in favour of a `ctx.modules` service, which is how every lazily loaded chunk in `0.13.1` resolved its external dependencies. On any host at or past that release `0.13.1` fails with `[dsh-better-sidebar] chunk "terminal": client module system unavailable` and loses the terminal, editor, and Mermaid panes. `0.14.0` injects `@deepseek-ai/dsh-client-modules` — which this repo has at `0.1.1-rc.1` — shares the plugin's own globals with its chunk copies, and drops the `dsh-client-web-react` and `dsh-client-schema-form` peers rc.8 removed. Its `node-pty` range is unchanged, so the override below applied as written for as long as the plugin shipped; [0.1.0-rc.33 withdrew the plugin](../simplification/2026-09-14-desktop-withdraw-better-sidebar.md) and retired that override with it.

`dsh-at-file` runs ahead of the author's newest npm release, and the reason is the split resolution below: a bundle's patch layer resolves installation-first through `resolveBundleDir` while its module resolves by the ordinary parent walk, which reaches a profile's own `node_modules` first. Shipping the registry's version against a profile that installed a newer one itself — the state the first machine to run this plugin is in — would pair one version's row with another version's code.
...
`pnpm-workspace.yaml` therefore carried `'dsh-better-sidebar>node-pty': '1.2.0-beta.15'`. The versions are API-compatible: `resize` gained an optional third argument and `useConpty` became a documented no-op. The override went out with the plugin in [0.1.0-rc.33](../simplification/2026-09-14-desktop-withdraw-better-sidebar.md); the core's own `1.2.0-beta.15` pin and its patch stay.
...
The seed writes to `$DSH_HOME` before the server does. It is confined to `profiles/desktop-shell/` and to links under `profiles/node_modules/`, and it only ever adds, but a shell that edits user data at all is a fact about this app the README states.

Customization a user made in their `web` profile does not follow them to the desktop client, which mounts `desktop-shell`. Nothing has to be cleaned up first: no published build ever seeded `web` — rc.16's `app.asar` carries no `profile-seed` — so no shared profile holds a name only the application can resolve. The built-ins need no action either, since the desktop ships them. What a user does have to redo is whatever they installed into `web` themselves, which is exactly the `dependencies` list in `~/.dsh/profiles/web/package.json`; `dsh plugin --profile desktop-shell add <package>` puts one of those in the desktop's profile. The README says this.

Every built-in in the payload comes from a committed archive rather than a registry, so each one is whatever its archive holds rather than whatever a registry would serve for the same version. A registry release at or past a shipped version is worth taking when it appears. The [vendored plugin reference gate](../process/2026-09-03-vendored-plugin-reference-gate.md) owns how those archives are named and kept in step with the manifest, the notices, and the README tables that restate them.

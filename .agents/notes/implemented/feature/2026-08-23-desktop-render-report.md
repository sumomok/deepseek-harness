...
| `x-dsh-render-report` | response, 200/500/504 | The whole `RenderReport` as percent-encoded JSON, at most 6 KB |

`x-dsh-render-landed-url` and both failure bodies are unchanged, so a caller that reads only those is unaffected by any of this.
...
Two cases pin the header's bound rather than its content: 100 pending requests with 2000-character URLs, 60 failures, and 50 console errors, and the same shape written in Chinese. Both assert the encoded header is at most `REPORT_HEADER_BYTES` and still parses, which is what a bound expressed in characters would fail.

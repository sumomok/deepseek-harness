...
Owner props are a seat-specific `SettingsTriggerActionOwnerProps`: the sidebar's `wide` state, the same currency the trigger content seat receives, plus `openSection(id)` — the shell's own component-local opener, which selects the section registered under that id and opens the modal. An occupant reaches a settings page directly rather than pressing the trigger beside it; an id no `settings.section` entry claims selects nothing of its own, and the panel opens on the first nav row. The onboarding coordinator already receives that same opener, so the seat takes a share the shell was already handing out.

Right alignment is structural rather than declared: the trigger is `flex: 1`, so everything after it sits against the row's right edge. The seat's wrapper and the renderer's own slot anchor are both `display: contents`, so an unoccupied seat contributes neither a flex item nor one of the row's 8px gaps — the row with no contributor is byte-identical in layout to the row before this change. The wide row is also the seat's hover surface: `.triggerRow` carries the 12px radius and the hover fill, and the trigger's own hover fill is suppressed inside it, so the trigger and an occupant light up together as one control. An occupant therefore draws neither a border nor a background of its own. The rail row is excluded from both rules, because there the 36px trigger circle is the only thing in the row and stays its own surface.
...
**Widen `SettingsTriggerOwnerProps` so both trigger-row seats carry the opener.** Rejected: the content seat draws the label and icon inside the button that already opens the panel, so an opener there is an affordance its occupant has no use for. A seat keeps its owner share to what its own occupant can act on.

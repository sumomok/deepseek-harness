...
Typert's host face builds one program per batch of discovered packages, rooted at every member's whole tsconfig file list. A package whose host entry declares a Cordis service is discovered on the host face; if that same package also has a `src/client` reaching the client runtime, both faces' `TypertContextMap` merges land in one program and the generator fails on a duplicated key (`agent`, declared by both `packages/core/agent` and the client runtime package's own client entry — `@deepseek-ai/dsh-client-runtime`, split up in 0.1.2-alpha.4). Every dual-face package in the tree avoids this by having no host surface (`content-frame`, `server-layout`, the `ui-*` rows) or no client-runtime reach (`client-modules`); a content router with a service and a column has both.

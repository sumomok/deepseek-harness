...
The shell now owns a surface that installs packages, and the audit for it is three lines of validation plus a native dialog rather than a review of the plugin that calls it. `apps/desktop-shell/README.md` states the protocol, which is what a second implementation would build against.
...
- [The render service](2026-08-22-desktop-render-service.md) is the first loopback service and the pattern this one follows; the two now share `apps/desktop-shell/src/loopback-service.ts`.

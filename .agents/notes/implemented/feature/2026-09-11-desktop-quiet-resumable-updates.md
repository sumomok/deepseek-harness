...
帮助 → 检查更新 keeps its place and its label because a menu item that vanishes is a feature nobody can find again. It runs the same silent check, and every answer it gives is one a click asked for: 「已是最新版本」, 「无法检查更新」, and — where the update is already under way — 「正在后台下载新版本」 or 「新版本已下载完成」. Those last two report where the update is rather than asking anything, because a click that produces nothing at all reads as a broken menu item. One manual dialog does still ask a question, and only on the download-page tier: 「发现新版本 / 去下载」. It is kept because that tier has no other way to hand the download over — the Settings entry can only report a version this build cannot install — and a silent check there says nothing unless the feed carries a red line above the running build. The in-place tier never reaches that dialog: when its check is interrupted, a click is answered 「无法检查更新」 and a silent check says nothing, red line or not.

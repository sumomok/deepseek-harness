...
`customCheckAppRunning` therefore kills `elevate.exe` **first and unconditionally**, and pointedly *without* `/T`: the installer is its child, so a tree kill would end the install it is trying to repair. Waiting for it instead would deadlock, since it is waiting for us. Only then does the wait-and-tree-kill loop for the app and the server run. `node.exe` is matched on executable path under a *verified* `$INSTDIR`, which is what makes killing one safe to do at all — the [app-running check](../bug-fix/2026-08-19-installer-app-running-check.md) owns what "verified" means and why the built-in check cannot be trusted with that value.
...
A hook that is not compiled in fails silently — `!ifmacrodef` simply does not fire — so both are proved by injecting `!error` into the macro body and reading the compiler's own reply: makensis stops with `Error in macro customCheckAppRunning on macroline 1` and, separately, `Error in macro customFinishPage on macroline 1`. That is the check to repeat whenever `build/installer.nsh` is renamed, moved, or newly ignored. The update receipt is proved from the log line the same launch writes (`first run after updating from …`) after seeding the state file with an older version.

...
- **3 — seam or hardcode.** One seam, and it names its two implementations: `export-pixels.ts`, which is the browser's, and the recording stub every case in `content-image-capture.client.spec.ts` injects, because jsdom has no canvas and this package takes no native one. Everything around it is hardcoded — the tag list, the order of the checks, and the two ways to the floor.
- **4 — boundary.**
...
- **1 — new surfaces: 3.** One module ([`image-report.ts`](../../../../packages/experimental/content-frame/src/access/image-report.ts)), one service in this package's inject list (`attachments`), and one method on the call table (`PendingCalls.reserveReport`).
- **2 — v0.** Store, then hand the reference to the table. No queue, no background write, no cleanup pass — that is the whole mechanism, and it is small enough that the one thing it was missing is a single `reserveReport` in front of it, taking the acceptance `report` already applies so that one post per waiting call reaches the store.
...
| Contract | Pixels are on disk before the call settles, and only for the one post that took the settlement of a call the table is holding for the posting tab. | A `tool/result` naming an object that is not there, and bytes nothing will ever read or remove — an object per concurrent post included. | permanent |
...
| Ceiling | It keeps what it is given. There is no expiry, no quota, and no delete-after-turn; a call that times out while its one reserved save is writing still leaves those bytes — one object per call, never one per post. | This package inventing a retention policy the store does not have. | deferred — trigger: the attachment store growing a collection policy of its own |
...
- **0 — what a settled principle already answered.** "No hardcoded tunables in plugins: deployment-varying choices are validated `Config` fields" — read as it is written, it says a choice that does **not** vary by deployment stays a constant, and names protocol constants and external specs as exactly that. All five are one or the other, and the sixth is derived from one of them, so the `Config` block gained nothing.
- **1 — new surfaces: 6.** Four protocol constants in [`wire.ts`](../../../../packages/experimental/content-frame/src/access/wire.ts) — `IMAGE_MEDIA_TYPE`, `IMAGE_PIXEL_BUDGET`, `RASTER_MIN_SIDE`, `MAX_EXPORT_BYTES` — the share of the read's deadline the export runs under, `EXPORT_WAIT_SHARE`, and `MIN_READ_TIMEOUT_MS`, the load-time floor on `readTimeoutMs` derived from that share. Zero `Config` fields.
- **2 — v0.** One constant — the byte cap — with the element's own size and no scaling in either direction. The probe is what said that is not enough: a 24-pixel `svg` rasterized at 24 pixels is not a picture anything reads, and the number it should be rasterized at already existed as the provider's own floor. The real-machine reads below said the same of a stored raster, which is why one floor covers both kinds and `RASTER_MIN_SIDE` is a side rather than an area.
- **3 — seam or hardcode.** Hardcoded, all six. Two deployments that would set any of them differently would have to be named, and cannot be: these are the provider's pricing, the attachment layer's projection, the format a canvas falls back to, a fraction of a deadline this package already splits, and the floor that fraction puts under the field a deployment does set.
- **4 — boundary.**
...
| Contract | Both kinds are carried up to the provider's own floor — a vector to its area, a bitmap by a whole multiple of its short side drawn with smoothing off — and both are then held to the pixel budget by the same geometry the attachment layer projects a request image with. | A picture arriving under the floor and misread, and an enlargement resampling the edges the page drew. | permanent |
...
A fourth reading, off a real console once the read had shipped: one 32 × 32 logo, read twice on 2026-09-04 and 2026-09-05, n = 2. Exported at its own 32 pixels it was described as a picture the logo is not. The model then enlarged that same stored PNG eight times over with nearest-neighbour and described it correctly. Two reads of one picture measure no accuracy either, and what they settle is narrower: the enlargement the provider does before pricing is not one the model reads, so a stored raster that arrives under the floor is misread whether or not the pricing scales it afterwards. That is what carried the floor from vectors to bitmaps, and what fixed the bitmap's way to it as a whole multiple drawn with smoothing off.
...
**Leaving a bitmap at its natural size, for the provider's own floor to enlarge.** What the read shipped as, and what the real-machine reads above reversed. The provider scales a small image up before pricing it, so a 240 × 240 icon is priced as 384 × 384 either way, and enlarging in the seat costs bytes on the post, on disk and in the request. The bytes are real; the conclusion drawn from them was that the model sees the same picture either way, and it does not.

**A fractional scale, landing on the floor exactly.** It reaches 384 on the short side instead of overshooting it — a 348 × 348 picture would export at 384 × 384 rather than 696 × 696, and a strip would spend less of the pixel budget. It also resamples: at a fractional scale one stored pixel lands across two exported ones, and what a small icon loses to that is the edges the read exists to show. A whole multiple keeps every one of them and costs only bytes, which the pixel budget already bounds.
...
The model can answer what a page draws, on the same channel and with the same `ref`s as every other read, and the `screencapture` path that [the sign-in note](2026-09-04-sign-in-pages-are-read.md) recorded now has a bounded alternative: one element instead of a display, at a size the export resolves from that element instead of the screen's.
...
A deployment on a text-only route gets the tool and a refusal naming its model — since [the model-switch note](2026-09-05-content-image-model-switch.md), that refusal is what a user who declines a card is answered with, and the card is the gate's other exit; a deployment with no attachment store gets neither the tool nor the route. Both are visible at the point of use rather than at load, because both are properties of the session and the composition rather than of the configuration block.

An export that never comes back ends as a refusal about this picture rather than as the channel's timeout sentence. The drawing gets an eighth of the read's deadline of its own, after the load wait's half and the settle wait's quarter, and that share is checked at load like the settle wait's: a `readTimeoutMs` under eight leaves the export a rounded budget of zero or one millisecond, and the row refuses to boot rather than refusing every picture as one the console did not draw in time; nothing in a browser cancels a `toBlob` or an `image.decode()`, so what the deadline ends is the seat's wait for it, and the promise stays pending for the life of the tab. The call settles at the seat rather than at the host, and the model is told `<ref> did not finish exporting within Ns.` instead of that the console went quiet.

## Testing

`tests/content-image-capture.client.spec.ts` drives every export decision against real documents with the drawing primitive injected — the four tags and the refusal of every other, the visibility, loaded, zero-pixel and taint refusals, the deadline a drawing that never returns runs out, the cut a tag the page spelled past the wire's bound is taken to, the media-type set, the byte cap, and the exact size and smoothing every rule resolves — a raster carried to the floor by a whole multiple, one already at it drawn one to one, a strip whose multiple runs past the budget, a downscale, and an SVG upscale. `tests/content-read-image-tool.client.spec.ts` pins the description, the parameter list and the header line verbatim against the real tool runtime, and drives both arms of the modality gate, the failure arm, the misreported arm, and the image block's attachment reference. `tests/content-image-report.client.spec.ts` holds the host half: the exact decoded bytes reach the store, the stored name follows the page and the element, the settled call carries a reference and no bytes, a refusing store settles the call with its own reason, the store is left untouched for a call nobody is waiting on, for one another tab claimed and for every post after the one that took the call, and the pixels are committed before the call settles rather than after. `tests/content-read-wire.client.spec.ts` covers the new parser and the route's byte arithmetic, `tests/content-read-routes.client.spec.ts` the route itself — including a waiting call whose posted capture reaches the store and settles it with the reference over real HTTP, and the load-time floor under `readTimeoutMs` — and `tests/self-contained-copy.client.spec.ts` walks the seventh tool definition and all twelve new strings. `src/` stays covered per file at 100%.

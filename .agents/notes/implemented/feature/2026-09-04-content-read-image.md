...
An export that never comes back ends as a refusal about this picture rather than as the channel's timeout sentence. The drawing gets an eighth of the read's deadline of its own, after the load wait's half and the settle wait's quarter, and that share is checked at load like the settle wait's: a `readTimeoutMs` under eight leaves the export less than a millisecond, and the row refuses to boot rather than refusing every picture with a deadline of nothing; nothing in a browser cancels a `toBlob` or an `image.decode()`, so what the deadline ends is the seat's wait for it, and the promise stays pending for the life of the tab. The call settles at the seat rather than at the host, and the model is told `<ref> did not finish exporting within Ns.` instead of that the console went quiet.

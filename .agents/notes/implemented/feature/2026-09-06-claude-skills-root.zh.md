...
只有 `complete: false` 时模型什么也收不到。`dsh-tool-skill` 只在快照完整时发布目录，因此按根降级只修好了 `ctx.skills.list()`、`get()` 与客户端选择器，而每一次 `agent/pre-step` 仍然什么都不发：`~/.claude/skills` 不可读时，模型连一个 `<available_skills>` 块都拿不到。现在 `packages/skill/tool-skill/src/index.ts:235` 的条件更窄——不完整的快照照常发布，除非它读到的模型可调用 skill 为零。这个计数算的是目录真正会携带的东西：它取自 `isModelInvocable` 过滤之后，因此幸存的 `disable-model-invocation` skill 不能冒充一份视图、把空替换放行。有东西可发的不完整快照，说明提供方只是报出了自己发现工作的一部分，它确实读到的那些 skill 是真实且可直接加载的；没有东西可发的那份则分不清“没有 skill”与“什么都读不到”。两种情况都不缓存，因此每一步都会重新观测。
...
发布出去的部分目录自称是完整的，而时好时坏的根会把它反复重发。每一条替换都带着上游的措辞「This complete catalog replaces every earlier available-skills list in this session」与「Use only names in this replacement catalog」，因此模型不只是没拿到被拒目录里的 skill 且没有诊断——它被告知自己原有的那些名字已经不存在了。目录不发什么因此同样承重：不带任何模型可调用 skill 的不完整观测什么都不发，因为那条空替换会对着可读的根仍然持有的 skill 说「No skills are currently available through the `skill` tool. Do not use names from earlier skill catalogs.」。只要 watcher 让某次变化失效，就会重发一次：把一个根的权限来回翻转六次、每次翻转后各失效一次并跑一步，追加了六条替换目录；而一直不可读的根只追加一条，之后连跑五步都保持安静。每条替换都是一条持久会话事件，也是一次 KV cache 断点，因此时好时坏的网络挂载或同步中的目录，每翻转一次就各付一份。

...
| Claim | Evidence |
|---|---|
| A call with no `tableConfig` is asked about without naming a column, and drawn in the table's own, with the scheme's hidden column left out | `examples/content-console/tests/snapshots/show-default-columns-turn/session.jsonl`; `apps/web/tests/component-surface-datasource.e2e.ts` and `.artifacts/web-e2e-component-datasource-default-{card,table}.png` |
...
| A filter matching nothing reaches the model as zero rows, not as an unreachable source | `examples/content-console/tests/snapshots/empty-datasource-turn/session.jsonl`; the same web scenario, whose scripted closing line can only be written from the tool's own zero-rows sentence in the live request |
| The model is told `gridItems` may be omitted, and what `currentPage` is | `packages/experimental/component-surface/tests/data-source.client.spec.ts`, which pins both sentences of the offer; `examples/content-console/tests/snapshots/show-chart-turn/tool-schemas.expected.json` carries the whole description |
...
The `examples/content-console` fixtures above are carried, not executed on this root: `examples/` is outside every workspace glob and no vitest config names that directory, so `show-default-columns-turn`, `empty-datasource-turn` and the refreshed `show-datasource-turn` cannot be replayed here — [that example's README](../../../../examples/content-console/README.md) records it, and where the suite should live is an open decision. What is executed for the claims they carry is the web scenario beside them, which drives the same code in a real browser.

`pnpm run build` and `pnpm run typecheck` green; `pnpm run doc-sync` green at 33 of 33 gates. `pnpm vitest run --coverage` over `packages/experimental/biz-backend` and `packages/experimental/component-surface`: 28 files, 687 tests, and 100% of lines, statements, functions and branches on every file. `pnpm vitest run --config vitest.web.config.ts apps/web/tests/component-surface-datasource.e2e.ts` against the built bundles: five cases, all passing. `pnpm run lint` sits on this root's own baseline of two oxlint errors in `examples/content-console/tests/content-console.snapshot.ts`, which stand there because that tree is no longer a workspace member and its imports resolve to no type — the same two errors, at the line numbers this change shifts them to.

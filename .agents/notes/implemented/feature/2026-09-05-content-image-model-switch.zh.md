...
**每会话一次决定，每会话一张卡。** 同一会话的路由决定经一个 `WeakMap` 串行，于是并行两次读取的第二次跑在第一次换完模型之后，并把那次变更当作自己的第一级读到——它不弹第二张卡就直接放行。用户以「不是某条路由」的方式作答，就被记进一个 `WeakSet`，该会话之后的读取不再问、直接拒；这个记号只压卡片，永不压放行。只有作出决定才记：被用户关掉的卡，以及为「永远够不到人」的被委派调用者竖起的卡，把问题就此定下。其余任何拒绝都不留记号，下一次读取照样再问——注册表已经不再持有的调用者、卡竖着时断掉的传输、抛错的应答者，以及 `NO_PROVIDER`：它说的是当时没有控制台连着，而不是有谁拒绝了；一分钟后开起来的控制台仍然该被问到。算数的那两个码（`ASK_CANCELLED`、`DELEGATED_CALLER`）是从拒绝值上按字段读的，不是按类判的，因为它经远程 waterfall 还原后才到达本包。
...
| 红线 | 记号只压卡片、永不压放行，且只有作出决定才置位：关掉的卡与永远问不到人的调用者把问题定下；当时没开着的控制台与断掉的通道都不算。 | 已经在控制台里换过模型的用户仍然被拒；以及当时控制台没开着，或卡竖着时重载了浏览器标签页，就让那次会话在整个进程余下的时间里再也拿不到这个提议。 | permanent |
...
**没送到任何人面前的卡不算作答。** 只问一次的记号只记决定，因此当时没开着的控制台、断掉的连接或出错的应答者，都会把这个提议留给模型的下一次尝试，代价是再多一张卡。

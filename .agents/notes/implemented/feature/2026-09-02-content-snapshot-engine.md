...
A bar the page named prints its name and its value. The value is the text the bar draws, and the number the page wrote only where it draws none: both bootstrap versions draw the percentage inside the element carrying the role, the row ends the descent, and those words reach the model there or nowhere at all. A screen reader reads the two the other way round and announces `25` over a bar drawn as `上传中，请稍候`; this read answers with the page. Only what the control draws for itself counts as its value, in a bar and in a field alike — the cancel button inside an upload bar says what to do about the upload rather than where it stands, and a row reporting `取消` as the quantity states something about the page that is not true of it. An unnamed bar reads as structure, because the percentage is then drawn in the text beside it and a row carrying neither a name nor a number would stand in front of that text.
...
Whitespace is the only thing taken out of the text a read prints, so a control character a page draws arrives in the snapshot as it stands. What answers for that is the seat carrying the read to a terminal, not the walk.

Five counter-examples to the rule that model-visible means readable are open. A role the page wrote that falls back to one ARIA does not name from its contents drops the words the element shows, unless that role carries a value instead: `<span role="bogus img">趋势图</span>` prints an unnamed picture. The fallback content of a `progress` or a `meter` is dropped on the strength of the specification saying no engine in use renders it, which jsdom cannot confirm either way. The tag-to-role mapping is reached only by an element the page wrote no role on, so `<meter role="bogus" aria-label="磁盘">` reaches the reader as nothing at all. A node the ladder does name — by a label the page wrote, or by text of its own — prints one row and folds away the controls drawn beside it, so the delete button of a labelled tree row reaches the model nowhere. And a layout table the page contradicts is read as the table it says it is, which stands it between the halves of the table beside it: the header those halves were drawn under then reaches the reader on neither of them.

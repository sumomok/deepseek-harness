...
A bar the page named prints its name and its value, because `aria-valuenow` on an empty `div` is the whole of what the page says about the upload. Where the page wrote no value at all, the value is the text the bar draws: both bootstrap versions draw the percentage inside the element carrying the role, and the row ends the descent, so those words reach the model there or nowhere. An unnamed bar reads as structure, because the percentage is then drawn in the text beside it and a row carrying neither a name nor a number would stand in front of that text.

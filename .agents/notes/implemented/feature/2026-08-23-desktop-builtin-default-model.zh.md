...
对这样的用户确实变了的是选择器那几行:无论他存着什么选择,被列出的只有 `deepseek-flash`,因为这份目录是组合配置项,不是逐用户的。存着的选择若指名一个已下线的模型,文本照常——DeepSeek 用 V4.1 Flash 为那些名字提供服务。图片在它上面发不出去,因为适配器把不在目录里的模型当作纯文本模型;而这个拒绝正是 `@haoran/dsh-vision-switch` 挂在这里的理由:在已经有过一轮的对话里,它经由选择器走的那同一个 `session.selectModel` 把会话切到 `deepseek-flash`,顺手把这个模型写回成存着的选择,图片随即发出。唯一失败的一轮是新对话第一条消息就带图片,报的是 `UNSUPPORTED_CONTENT`;同一张图再发一次就切过去并发出,于是一个已下线的存量选择在有人第一次发图时自行修好。他自己的 `cordis.patch.yml` 不受影响,播种从不编辑用户 patch 层。这条路径要求 `vendor/` 里的 `@haoran/dsh-vision-switch` 是 0.2.2 或更新的版本:0.2.0 用客户端契约已经改掉的旧名字(`imageIds`,现名 `attachmentIds`)去读草稿的附件 id,而这一句正好在它所替换的 `submit` 的第一行、且在它自己的 `try` 之外,于是在它装过的会话里每一次发送都死在输入框自己的处理函数里——发送按钮、Enter 与 Cmd+Enter 全都没有反应,纯文本和图片一样;而输入 shell 被重建过的会话则留着一个没被改写的 `submit`,图片就这样不切换地发了出去。0.2.2 把这个读取的类型取自契约自己的 `InputState`,按改写过的 shell 实例给每次安装记键且绝不给同一个实例包第二层,每次尝试最多调用一次原始 `submit`——输入框自己的 `submit` 在把发送交出去之后还会继续做事,因此从它那里抛回来的错误只会被报出来,而不是用同一份草稿的第二次发送去回应——并在闸门内部出任何差错时把这次发送原样转交出去。它的触发条件也从「图片」放宽到「任意附件」:输入状态只发布附件 id,插件持有的服务面分不出 PDF 和照片,因此在退役选择上挂一个文件,走的是和图片相同的那条切换。

...
- Under `judged`, a set of steps the waterfall allowed runs with no card, and a `deny` from anywhere on the chain still refuses it. A reviewer's own `ask` reaches the user in the reviewer's words; the step list is on the call card either way, because `presentCall` is unchanged.

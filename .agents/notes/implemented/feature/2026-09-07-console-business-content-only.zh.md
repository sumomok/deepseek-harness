...
**输入框占位文案在两种状态下都由同一对规则读作「说说要做什么」。** `InputBar.tsx` 为英雄态与已建立态渲染的是同一个 `[data-composer-placeholder]` 元素——`ConversationRoot.tsx` 改的只是喂给它的 locale key——因此 `font-size: 0` 加一个 `::after` 就覆盖了两者。替换后的文案带的是 `var(--dsh-content-font-size, 14px)`，也就是输入框卡片自己设置的那个 token，所以它继续跟随设置里的字号偏好，而不像英雄区标题那条规则那样冻结在一个像素值上。这个元素本来就是 `aria-hidden` 的，兄弟节点 `[data-composer-input]` 上的 `data-placeholder` 与 `aria-label` 则有意保持不动。

## Alternatives considered

**`ui-chat.transcriptView`，那个看上去就是为此而生的设置。** 它一个目标都够不到。它的取值联合恰好是 `'normal' | 'compact'`（`packages/client/ui-chat/src/chat-settings.ts:12`），而 `'compact'` 已经是默认值，因此控制台今天就已经停在隐藏最多的那一档上。`compact` 是折叠而不是隐藏：成员会拿到 `hidden="until-found"`，浏览器页内查找的 `beforematch` 会把折叠盘重新打开（`searchable-hidden.ts:21-28`）。它也从不作用于运行中的轮次——`ChatNodeSeat.tsx:63-69` 的 `processWindowReady` 要求 `turnClosed` 与完整加载的历史。而且它从构造上就会让 `system-prompt`、`turn-process` 与 `turn-tail` 保持可见：这三者都属于 `TURN_PROCESS_INDEPENDENT_KINDS`（`contract/turn-process.ts:20-33`），也就是整张隐藏清单里除工具行与思考行之外的全部。组合层也钉不住它：`SettingsRegisterOptions` 只提供 `base`，而 `base` 输给用户层（`packages/settings/settings/src/index.ts:739-748`），并且 `ui-chat` 注册它的 schema 时没有给 `base`，也没有暴露 `Config`，所以为它写一行 cordis.yml 根本没有键可写。

...
`apps/web/tests/server-sidebar.e2e.ts` loses the browser scenarios written for those rules by hand rather than by revert: the describe that seeded one closed turn and read each hide back, its `seedProcessTurn` seeder and seeded-string constants, the `expectGuardHidesSelector` and `expectSwappedPlaceholder` helpers no other scenario calls, the imports only that seeder needed, and the module doc's paragraph announcing the block. The commit that finished those scenarios also carried two unrelated ordering-barrier fixes to the workbench-click and temporary-section scenarios, which a revert would have taken with it. Two assertions from the same commits stay, because they now pin the opposite fact: the zero-Workspace scenario still reads the inert composer's placeholder for its own font size and the absence of an `::after`, which is what the stylesheet leaving that element alone looks like from the browser.
...
- The two rendered-text banned-word screens in `apps/web/tests/server-sidebar.e2e.ts` regain the reach the hides had taken from them — `workspaceWordsInChat`, which reads the chat column's `innerText`, and the landing scenario's whole-`body` read. `display: none` keeps a row out of rendered text, so banned vocabulary inside a hidden process row passed both unread. The third screen beside them reads `sidebar(page).innerText()` and is scoped to this package's own chrome by design; it never reached the conversation column, so nothing about it changes here.

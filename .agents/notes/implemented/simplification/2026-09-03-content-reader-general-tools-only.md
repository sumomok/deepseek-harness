...
- **Kept.** Two readings no specification defines survive, each with what retires it. A pointer cursor marks a click target (`looksClickable`): CSS is how a page tells a user this is a thing to press, and it retires when a seat can hear a framework's own click listeners. The `label` a page draws in front of a field it ties to nothing names that field (`drawnBefore`): HTML says a `label` labels a field and leaves which one unsaid, and it retires the first time it is measured naming a field after words that are not its label, a wrong name being worse than none.

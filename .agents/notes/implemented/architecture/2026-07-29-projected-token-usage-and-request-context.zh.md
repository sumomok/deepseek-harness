...
这是刻意接受的结果。占用率百分比是面向用户的参考数字。写下本条时它也确实无人读取：自 rc.33 fork 补丁起，`compaction-basic` 在计量以提供方用量锚定时读该投影作为自动压力触发依据，其余状态才读 `measure()`——见[自动压缩策略位](../feature/2026-09-14-auto-compaction-policy-seat.zh.md)。TUI 状态行一直以这种方式计算占用率，即用 `measure()` 总量除以为所选模型单独解析出的容量；因此在这里做成原子版本才是异类，而不是常态。

...
**The manifest `name` is the whole discriminator, and rewriting it is the one exception to the seed's own rule.** The rename runs only where the new profile has no `package.json` — the same predicate `initDesktopProfile` answers `created` with — and only where the old directory's manifest reads `dsh-profile-desktop`, which is what this shell wrote and what upstream's `@deepseek-ai/dsh-desktop-runtime` never is. Afterwards the manifest's `name` is rewritten to `dsh-profile-desktop-shell`: everywhere else the seed leaves an existing file alone, and this one field names the directory the manifest belongs to, which pnpm reads for every `dsh plugin --profile desktop-shell` command run there. A launch that finds a manifest already at `desktop-shell` reads that same field, because `dsh-profile-desktop` there is a rename whose rewrite did not land, and nothing on the machine states that afterwards; the rewrite is completed then, and any other name is left alone.
...
**The Updates tab moves with the rename, and a rollback record an earlier build wrote still opens.** `@haoran/dsh-plugin-updates` 0.2.1 — the vendored tarball in `apps/desktop-server/vendor/` — compiles `DESKTOP_PROFILE = "desktop-shell"` into its host half, reads `$DSH_HOME/profiles/desktop-shell/package.json` and `web-migration.json`, and sends `profile: "desktop-shell"` to the plugin-admin service, which is one of the two names it accepts. Its rollback journal is the one piece of its state outside any profile — `$DSH_HOME/dsh-plugin-updates/last-update.json` — so the rename does not move it and a record written before this build still reads `profile: "desktop"`; the plugin maps that value onto `desktop-shell` as it reads and writes only the new name, so the undo step survives the upgrade. The built-in plugins were never at stake, because they are seeded rather than installed, and nothing else in the payload names the profile. 0.2.1 is built for this name alone: a shell that still boots `desktop` needs 0.2.0.

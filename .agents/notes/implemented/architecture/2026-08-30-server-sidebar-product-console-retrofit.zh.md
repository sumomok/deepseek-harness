...
**拖拽重新排序，以及原生右键菜单来做改名/移除。** 两者都是任务书本身的字面要求，且各自都带有明确的降级条款（「若实现体量失控，降级为右键菜单「上移/下移」」）。考虑到本次改动本身的体量已经很大（一次完整的外壳重写、一次 schema 迁移、一次路由语义变更、加一次四行的去术语化排查，全部塞进一个 PR），两者当时都被降级：重新排序降级为上移/下移图标按钮，改名/移除降级为原收藏菜单已经在用的悬停显现图标按钮习惯。两处降级都没有触碰底层数据模型（`order` 本来就是一个普通的可排序字段）——这正是后续一次产品修正里，重新排序这一半能换成真正的 HTML5 拖拽（见「Decision」）而不需要数据迁移的原因；改名/移除的右键菜单降级依然保留，因为为这两个偶发动作再引入第二种交互模式依然没有正当理由。

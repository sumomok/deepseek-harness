...
**No seam was built between this package and the gate.** Two settled principles say no, and the first is enough. Out-of-repo plugins reach this repository only as `pnpm pack` tarballs vendored into a deployment closure, never as a link — so a repository package cannot depend on the gate at all, in either direction of the import. And a capability seam is complete or it is not built: there is one consumer today, and the last time this same problem was answered it was answered with names — `show_chart`, `job_output`, `content_show` and `web_search` joined the gate's defaults at 0.2.0, one of them this package's own tool.
...
**3. Seam or hard-wired? Hard-wired.** No registry, no service, no per-tool metadata. Cutting a seam would need two consumers that are really about to change, and there is one; the gate's answer to the same problem at 0.2.0 was four more names in its own defaults, `content_show` among them. Review trigger: a second repository package needing its tools classified, or upstream `ToolDefinition` growing a review field that any gate could read.
...
**Put `content_act` on `readOnlyTools` too.** It has a human gate of its own: `registerActApproval` returns a card naming every step by label, and the gate's own default list carries `ask_user_question` for the same reason — a tool whose whole effect is to put something in front of the person at the keyboard. Judging `content_act` costs a round trip on the good path, and an `ask` verdict costs two things, because `alwaysAsk` and an `ask` verdict both return without calling `next()` (`src/index.ts:442-446`, `:527-543`) and this package's own listener never runs. The smaller cost is that the step-by-step card is replaced by one sentence. The larger one is that `approvals.ask(exec.callId)` is reached only after that delegation (`src/access/act-approval.ts:45-47`), so it never records the call, and the tool body then refuses every call carrying `dialogs: "accept"` — `confirmed()` spends a record that was never made (`src/access/act-tool.ts:228-230`, `src/access/dialog-approvals.ts:52-54`). A person agrees on the judge's card and the steps are refused anyway. The gate closes this for 0.3.0 by delegating before it asks — `await next()` first, a downstream deny or ask returned as it stands — so the paragraph above prices 0.2.0 as shipped. The owner ruled on 2026-09-06 that the judge reviews `content_act` and that prompting a person is reserved for a dangerous page action. That cost is the price of the ruling and is recorded here so the trade is visible if it is revisited. See [the content-act note](../feature/2026-09-02-content-act-page-steps.md) for what the card contains.
...
- The five content reads and `content_show` skip the judge on a console that passes the overlay. `content_act` is judged, which costs one round trip per set of steps; on an `ask` verdict it also replaces the step-by-step approval card with the judge's sentence and, on gate 0.2.0, refuses any call carrying `dialogs: "accept"` after the person has agreed.
...
Three changes belong to the gate and are being made there for 0.3.0: the judge reading each tool's registered description, the judge route becoming a setting rather than two required `Config` fields, and the gate delegating before it asks so that a listener behind it still runs on an `ask` verdict. The second retires the `provider` and `model` lines this overlay restates; the third closes the `dialogs: "accept"` refusal priced above. None is blocked on anything in this repository.

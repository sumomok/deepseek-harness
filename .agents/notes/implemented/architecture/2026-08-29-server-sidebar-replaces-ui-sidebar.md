...
**Replace the whole sidebar, the same way [`dsh-experimental-server-layout`](../../../../packages/experimental/server-layout/README.md) already replaces `dsh-client-ui-layout`.** An overlay disables the `ui-sidebar` row and inserts `@deepseek-ai/dsh-experimental-server-sidebar` in its place. `ServerSidebarRoot` re-declares child slot registrations by type-only import of `@deepseek-ai/dsh-client-ui-sidebar/client` rather than restating the `SlotMap` augmentation, so any package registering into one of the reused keys keeps working with zero changes. At the time of this decision that meant all five of ui-sidebar's child slots, reused alongside a verbatim port of `SidebarRoot`'s whole behavioral contract (the 56px collapsed rail, the 150ms wide-content fade-out, the `.railIn` live-collapse-only animation, and the pointer-driven scrollbar linger); [`2026-08-30-server-sidebar-product-console-retrofit.md`](2026-08-30-server-sidebar-product-console-retrofit.md) later dropped `sidebar.workspaces`, the New Session button, and the collapse rail from this package's own contract, leaving four reused keys — `sidebar.brand.mark`, `sidebar.brand.name`, `sidebar.settings`, `sidebar.footer.action` — as the current shipped set.

Two engineering points this decision turns on:

- **The runtime registration and the TypeScript declaration are reused differently.** The `ctx.slots.register({ children: {...} })` call is restated in full — declaring is claiming, and every registrant states its own children regardless of whether another package already stated the identical spec. The `SlotMap` TypeScript interface augmentation for the reused keys is picked from a type-only import of `@deepseek-ai/dsh-client-ui-sidebar/client` instead: a second identical-type augmentation is legal (`ui-slots` only rejects a *conflicting* redeclaration), but importing the type keeps one documented source of truth for a registrant's compile-time contract regardless of which sidebar composed it, and the import is erased at build (a `type`-only import creates no module-graph request, so it costs nothing in the bundle).
...
A composition wanting this replacement carries an overlay disabling `ui-sidebar` and inserting this package (`overlay/customer.patch.yml`, typically alongside content-frame's own overlay, so the navigation group has something to list). `dsh-client-ui-sidebar` itself is untouched — any deployment not composing this overlay is unaffected, and the official package's own tests, snapshots, and future changes need no awareness of this replacement existing.

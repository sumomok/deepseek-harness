...
For proactive pressure, compaction-basic reads the latest durable request route, resolves its adapter capacity and exact-target policy, and scales ratios into a `ResolvedCompactSpec`. Capacity resolution is unchanged, but since the rc.33 fork patch the numerator compared against that spec is the context-pressure projection whenever the meter anchors on provider usage, and a mounted policy service can replace the ratio at run time — see [the automatic-compaction policy seat](../feature/2026-09-14-auto-compaction-policy-seat.md). It performs this resolution on every check, so a provider or model switch in one session changes capacity and policy immediately. An absolute retained budget that is not below the scaled threshold fails when the target capacity first makes that comparison possible.

...
**A mark has one written form, shown everywhere it is named.** `MARK_EXAMPLE` — "where the read printed e7 clickable {class: el-icon-delete}, pass ref "e7", label "" and mark "el-icon-delete" — the tokens alone, without the braces and without the "class:" printed in front of them" — is the parameter's description and both refusals about it. The wire refuses a mark carrying a brace or a leading `class:`: those are what the listing puts around the tokens, so either one is the printed row copied whole. A page could put a brace in a class attribute — nothing that draws one does, and such a row would be refused with the example rather than failing the seat's check a round trip later.

...
**标记只有一种写法，凡是提到它的地方都给出来。** `MARK_EXAMPLE`——「读取打出 e7 clickable {class: row-action danger} 时，传 ref "e7"、label ""、mark "row-action danger"——只要 token 本身，不带花括号，也不带印在它前面的 "class:"」——既是参数描述，也是关于它的两条拒绝语。wire 拒掉带花括号、或以 `class:` 开头的标记：这两样是清单包在 token 外面的东西，出现即说明整行被原样抄了进来。代价是确定的：class 属性里真带花括号的元素**永久不可操作**——清单为它打出的那串标记是步骤唯一能带的东西，而带着它的每一步都会在这里被拒，拒绝语给的补救模型还执行不了。只是没有哪个画页面的东西会这么写 class，而这条规则拦下的是模型在「天天遇到的那种行」上会犯的错，所以这笔账划得来。wire 确实吸收的是空白：标记按「token 用单个空格连起来」来读，与座位算它的方式一致，于是同一串 token 多一个前导或中间空格仍是同一个标记，而不是一个来回之后在页面上失败。

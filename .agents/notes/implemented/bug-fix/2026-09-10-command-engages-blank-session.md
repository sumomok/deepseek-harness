...
`markEngaged` also latches `engaged`, which `handleBlank` consults. Without it an `api-session/added` frame minted before the command landed, or a list pull that raced it, would raise the bit back: `handleSessionAdded` passes the frame's own `blank` straight through. `handleRunning` latches where it lowers the bit on the first running turn, which outlives itself the same way.
...
Two vendored fork plugins keep the default: `/review` in `@haoran/dsh-llm-permission-gateway` 0.3.1 and `/screenshot-logout` in `@haoran/dsh-screenshot` 0.5.1 both configure rather than contribute. Typing either as a fresh session's first line engages it — the hero closes and the session is listed with the command card — which is the outcome `/btw` produces: reachable only by typing, with the host fold and the client mirror agreeing and no orphan row. rc.31 ships them as they are. Declaring `engages: false` in those plugins is the follow-up, and a plugin may declare it safely: `normalizeDefinition` assembles the registered definition from the members it knows, so an older host drops the field instead of refusing the registration.

Sessions that ran a command before this build and were checkpointed keep `blank: true` for good. `viewCheckpoint` in [`session-projection`](../../../../packages/session/session-projection/src/index.ts) discards a row only when its `ver` does not match the unit's `stateVersion`, and `restore` seeds from a usable row and folds only the events past that row's `seq` — so reopening the session refolds nothing that predates the checkpoint, and a session whose only engaging event is older than this build stays hidden from the list and reusable as the workspace's New Session, with the command card in it, until a new engaging event lands. That is the accepted cost of holding `stateVersion` at 1, which is what keeps `lastPromptAt` for every session that is never opened again.

An engaging command starts no turn, so nothing pushes the flip to the other readers of the list: `api-session/status` carries the running bit alone and there is no `api-session/updated` event at all ([`remote-events.ts`](../../../../packages/api/session-controller/src/remote-events.ts)). A second tab or client — and this client for a session it never opened — keeps `blank: true` in its list row until its next `session.list` pull, and until then `connectWorkspace`'s reuse scan ([`navigation.ts`](../../../../packages/client/ui-workspace/src/client/navigation.ts)) can hand that session back as the workspace's New Session with the command card already in it. Known limitation of this patch across tabs and clients; not fixed here.

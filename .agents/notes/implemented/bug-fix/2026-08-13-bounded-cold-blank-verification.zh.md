...
`dsh-api-session-controller` 注册 `sessionListMetadata` 投影，其中包含 `blank` 与 `lastPromptAt`。已附加摘要直接用同一组函数折叠实时日志。`blank` 只在 `turn/start`、或未被命令声明为会话配置的 `command/run` 上从 true 单调变为 false（[命令转正 note](2026-09-10-command-engages-blank-session.zh.md)）；`lastPromptAt` 只在来源 kind 为 `user` 的 `user/message` 上更新。

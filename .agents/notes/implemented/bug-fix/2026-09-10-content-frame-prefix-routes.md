...
- **The browser lane still does not drive a read under the prefix.** `apps/web/tests/base-path.e2e.ts` boots the shipped shell plus `server-base` and `auth-gate` behind the stripping proxy, and its standing assertion — no request lands off the prefix — would have caught this had content-frame been in that composition. It is not, and adding it is not a small change: the content column needs all four rows (`server-layout`, `content-surface`, `content-column`, `content-frame`), and `server-layout` replaces `ui-layout`, which is the shell that scenario's existing assertions are written against (the workspace picker, the boot path). Driving an actual read then needs the seed-and-execute shape of `apps/web/tests/content-read-hidden.e2e.ts` — a spliced `content/shown` plus an open call, and `ctx.tools.execute` under the same call id, so no model round trip is involved. The two halves exist; what is missing is a scenario of their own that owns both, which is where this belongs rather than bolted onto a scenario about the shell's own URLs.

...
The desktop composition's `@haoran/dsh-auto-compact` (0.2.4, vendored as a tarball) moves the trigger out of the turn. No harness package changes.
...
A delegated run reaches that idle transition on one of two routes, and a compaction is worth having on neither.

**Foreground** (`run_in_background: false`): the parent's tool call settles on `await child.whenIdle()` inside the in-process driver (`packages/subagent/subagent-in-process-driver/src/index.ts:178-182`), and a compaction started from the child's idle transition claims the maintenance phase synchronously, replacing `activityDone` — so the tool call does not return until the whole summarization finishes, in the middle of the reply the user is watching, and the child is disposed straight after with the summary unread. Measured against a real `AgentLoop` with an 800 ms stand-in summarizer, three runs: a skipped delegated conversation settles in 3 ms, a conversation that does compact in 803/804/804 ms with one `compactNow` call.

**Background**, which is what ships: the standard preset configures both in-process delegation tools `backgroundMode: continuable` (`packages/preset/agent-presets/presets/standard/agent.cordis.yml:187` and `:198`; the `one-shot` rows at `:210`/`:219` are the `disabled: true` codex and claude-code providers, which run no agent in this process), and `runInBackground` then defaults to `options.continuable` (`packages/subagent/tool-subagent/src/index.ts:303`), so the tool returns a subagent id at inbox acceptance and waits for nothing. Compacting there holds up no reply. It still delays the child's result behind the summary, queues a later `send_message` behind it, and leaves a child never messaged again holding a summary no one reads.

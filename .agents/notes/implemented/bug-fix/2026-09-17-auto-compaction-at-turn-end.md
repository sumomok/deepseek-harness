...
The desktop composition's `@haoran/dsh-auto-compact` (0.2.1, vendored as a tarball) moves the trigger out of the turn. No harness package changes.
...
No context event reports a cancellation and no projection publishes the end reason, so the signal is the closing event itself, observed through `ctx.on('session/event')` as it commits rather than by scanning the log back. Each attempt also carries its own `AbortController`, aborted on `agent/disposed`; that event is emitted after driver quiescence, so it is the second line of defence, not the first.

**Every delegated agent.** `agent/status` is scope-filtered but the listener is on the plugin context, so it receives every agent's transition, subagents included. Their sessions are usually torn down as soon as they answer and their settlement waits on `whenIdle()`, so a summary there spends a model call on history nobody reads again and delays the parent. `ctx.agents.roots().includes(agent)` is the filter, following the `schedule` plugin's precedent.

...
匹配看的是内容,不是文件字节。每一条顶层条目由 `js-yaml` 在 `FAILSAFE_SCHEMA` 下单独解析,解析被拒的条目——标签、重复键、制表符——原样留下。

`js-yaml` 就是 loader 读 patch 层用的那个解析器。它在那里用的 schema 是 `yaml.JSON_SCHEMA.extend(JsExpr)`:`vendor/include/src/index.ts` 里的 `entryListSchema`,经 `packages/boot/app-boot/src/index.ts` 的 `userPatchesSchema` 抵达。failsafe schema 不是那个 schema。它拒绝的是那个 schema 所拒的一个超集——`!!js` 与全部 JSON 标签,`!!int`、`!!bool` 在内——并且把它接受的每一个标量都读成字符串,而 loader 还会把 `5`、`true`、`null` 解析成数字、布尔与 null。这两处差异在本方向上都是安全的:被拒就是原样留下,而三条种子行里的每一个取值都是两种 schema 读法一致的普通字符串,所以这里匹配成立,拿到的就是 loader 在那里会读到的东西。两处差异里有一处会落到本模块的行为上,并且钉住了:在 loader 的 schema 下,`null` 标量是一个 null 节点,而条目里任何位置出现 null 都会让它在这里判为不可读,于是同一条在 failsafe schema 下解析得开的条目,改由它的文本来识别。把 schema 换掉,那条用例就红。

解析读出来的东西按每个映射的键排序后序列化,再与那两行相比,所以同样的字段配同样的取值,不论当初按什么顺序写下都算匹配。顺序不属于一份抄件的身份——把这一行拿掉,正是让随包分发的预设顺序回来的办法。

空节点是本模块要应付的另一种 null。`js-yaml` 把写了键却没有值、或者短横后面什么都没有的序列项,读成那个位置上的一个 null,与 schema 无关——所以 `isPatchValue` 校验的是整个拿到手的节点,而不是只看它的顶层,因为序列化器用 `Object.entries` 读映射,遇到 null 会抛。带着这种 null 的条目判为不可读,而不可读的条目本模块本来就有答案。再往后,整个退役跑在一个 catch 之下,凡是没被就地处理掉的都变成一行点名文件的跳过日志:一份应用里没有任何东西能编辑的 patch 层,不可以让壳打不开。
...
网关那一行两种形态都会被匹配——包的 `- insert:` 与以 id 为目标的 `- id:`——条件相同:每个字段都是随包分发的那个值。把网关行与别的行一起插入的条目,会被认出携带了它,并且永不删除——对一条不是种子行的行来说,认出它的全部作用就在于此:换来一条点名它的跳过日志。以 id 为目标的那一种并不是看上去那样一份无害的重复:这样一条 patch 会替换整行的 `config`,所以一份带着 0.1.3 的 `provider` 与 `model` 的副本会把那条裁判路由钉死,并丢掉此后某个版本在自己那一行上设的每一个 `Config` 字段。任何一处有出入的行都是文件主人的,会留下,并在日志里给出理由:`skipped cordis.patch.yml: the permission preset table is not the one this shell wrote; left exactly as it is`。网关那一行上的裁判路由也算——有人改过的 `model` 就让那一行成了他的。发生移除时打印 `retired the permission preset table from cordis.patch.yml`,来自新增的 `SeedReport.retired`。
...
**用手写的逐行扫描器而不是解析器来认这两行。**大约 140 行就能不带依赖地把条目读成映射、序列与简单引号标量。可是 `js-yaml` 本来就是 `apps/desktop-shell` 的运行期依赖——`src/theme.ts` 与 `src/updater.ts` 都在 import 它——而且正是 loader 读 patch 层用的那个解析器,所以用它不多一个包、不多一个版本、asar 也不多一克;手写换不到任何东西,却要背上维护。扫描器也恰是本模块最不该无人测试的那一半:`apps/` 不在覆盖率门禁内(`vitest.config.ts` 只 include `packages/*/*/src`),没有门禁会逼它的拒绝分支跑起来,而正是那些分支在决定要不要改写用户的配置。凡是扫描器的规则与解析器不一致之处——块标量、值里带 ` #` 的行——解析器在构造上就是对的那个,因为 loader 将来读那个文件用的就是它。

...
`js-yaml` 就是 loader 读 patch 层用的那个解析器。它在那里用的 schema 是 `yaml.JSON_SCHEMA.extend(JsExpr)`:`vendor/include/src/index.ts` 里的 `entryListSchema`,经 `packages/boot/app-boot/src/index.ts` 的 `userPatchesSchema` 抵达。failsafe schema 不是那个 schema。它拒绝的是那个 schema 所拒的一个超集——`!!js` 与全部 JSON 标签,`!!int`、`!!bool` 在内——并且把它接受的每一个标量都读成字符串,而 loader 还会把 `5`、`true`、`null` 解析成数字、布尔与 null。这两处差异在本方向上都是安全的:被拒就是原样留下,而三条种子行里的每一个取值都是两种 schema 读法一致的普通字符串,所以这里匹配成立,拿到的就是 loader 在那里会读到的东西。于是这处差异透过本模块自身的行为观察不到,也没有用例钉住它;用例钉住的是:带着数字、布尔或 null 的条目照样解析得开,照样什么都不匹配。`44f6132b6d` 的提交信息把那个 schema 说成了默认 schema,以本段为准。

...
That last step does not hold, and the machine settled it afterwards: sampling the process table at 250 ms through a failing install recorded five `old-uninstaller.exe` runs and no process check at all, and `$INSTDIR` read `D:\soft\DSH Desktop` straight off the wizard. `CHECK_APP_RUNNING` is skipped on the installer's side entirely — `${ifNot} ${UAC_IsInnerInstance}` in `installSection.nsh`, against a `perMachine` install whose section runs in the inner instance — so `appCannotBeClosed` arrived from `uninstallOldVersion`'s retry limit instead. [The uninstall staging runs past MAX_PATH](2026-08-19-windows-update-max-path-uninstall-loop.md) owns that chain and the fix for it. The over-match described below is real, reachable, and worth refusing; it is not what produced this dialog.

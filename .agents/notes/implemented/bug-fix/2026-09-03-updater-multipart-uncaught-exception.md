...
**Handle it in [`src/updater.ts`](../../../../apps/desktop-shell/src/updater.ts) instead.** The event is thrown by the emitter inside electron-updater's own callback; nothing the shell subscribes to sees it, and the updater's fallback already ran — the full download it fell back to is what finished. The shell has no seam here.

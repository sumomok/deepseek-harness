...
`dsh-api-session-controller` registers `sessionListMetadata`, a projection containing `blank` and `lastPromptAt`. The attached summary folds the same functions directly over the live log. `blank` changes only from true to false, on `turn/start` or on a `command/run` the command did not declare as session configuration ([the command-engagement note](2026-09-10-command-engages-blank-session.md)); `lastPromptAt` changes only on a `user/message` whose source kind is `user`.

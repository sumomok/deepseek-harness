...
[`apps/desktop-shell/src/updater-log.ts`](../../../../apps/desktop-shell/src/updater-log.ts) 掌管 electron-updater 被允许写下什么。`updaterLogLine(channel, message)` 渲染出一条记录，或者返回 `null`，[`src/updater.ts`](../../../../apps/desktop-shell/src/updater.ts) 用它拼出 `built.logger`——整个决定就是一个纯函数，跑起来完全不需要 electron。

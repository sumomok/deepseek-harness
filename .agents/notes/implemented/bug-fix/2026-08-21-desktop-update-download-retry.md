...
`apps/desktop-shell/src/download-retry.ts` holds the whole decision: `classifyDownloadError`, `describeDownloadError`, `RETRY_DELAYS_MS`, and `withRetry(run, delays, { onRetry, sleep })`, whose `sleep` is injected so the plan is exercised against an instant clock. `updater.ts` and `progress-window.ts` import electron and are not unit-testable; nothing in the policy needed to be there.

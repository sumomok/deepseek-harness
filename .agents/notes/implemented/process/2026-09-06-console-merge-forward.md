...
`packages/client/modules/src/index.ts` mints the combo route root-absolute and projects it wherever something other than the process asks for the address; the served response map and the Host route stay keyed by the root-absolute route. `pageUrl()` strips the leading slash for the graph row and the batch descriptor, both of which the page resolves against its own deployment base.

The `sourceMappingURL` a combo script carries takes a different projection, because a source-map reference resolves against the generated code's own source origin rather than against the document (TC39 source-map specification §11.1; Chromium's `SourceMapManager` completes it against the script's URL). Neither whole address serves both deployments: a root-absolute one loses the path prefix, and a page-relative one is appended to the script's own directory and asks for `/plugins/plugins/??…`. `comboUrl()` mints the script and its map on one path that differs only in the query, so `scriptRelativeMapUrl()` stamps that query alone and the browser keeps whatever path served the script. `packages/client/modules/tests/node-half.client.spec.ts` asserts where the stamp resolves rather than how it is spelled, from a site-root base and a `/console/` base alike.
...
Two gate facts a reader of this branch's CI needs. `pnpm run verify-client-domain-graph` exits 1 on three violations, all `packages/client/ui-conversation/src/client/skeleton/InputBar.tsx` importing `../input/editor/…`; `git diff 467f171a9b..HEAD` is empty for that package and for the gate script, so it fails identically on the parent. And for `config-catalog`, `capability-seams`, `event-producer-consumer`, and `persistence-catalog` only the English side is generated — the Chinese sides are mirrored by hand, and `verify-translation-pairing` compares git blob hashes rather than content, so no gate can detect a Chinese side that has drifted from what the generator would write.

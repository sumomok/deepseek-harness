...
`apps/web/tests/server-sidebar-views.overlay.yml` no longer disables `@deepseek-ai/dsh-client-ui-workspace`. On rc.1 `dsh-client-ui-conversation` injects `uiWorkspace`, so disabling that row leaves the entire conversation column pending and the page renders nothing. The overlay is now what its own header claims — its sibling plus the component rows — and the workspace vocabulary it was disabling the row for is what `terminology-guard.ts` already hides, as the sibling composition's own scenarios assert.

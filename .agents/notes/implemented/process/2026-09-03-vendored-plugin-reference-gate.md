...
Nothing here checks the lockfile, and one re-vendoring shape needs it to. Replacing a tarball **under its existing filename** leaves `pnpm-lock.yaml` recording the previous archive's `integrity`, and pnpm does not notice: a plain install, `--fix-lockfile`, and `--force` all keep the recorded resolution and reinstall the old bytes from the store, so the vendored file and the installed package silently disagree. Evicting the entry is what re-hashes it — point the specifier at another filename, install, point it back, install again — and the proof is the `integrity` line moving to the new archive's `sha512`. Every re-vendoring before `@haoran/dsh-btw` 0.1.0's `private: true` fix carried a new version in the filename, which is why the trap had not been hit; `verify-third-party-notices`, now a `doc-sync` leaf, does not cover it either.

Nothing here checks the lockfile, and one re-vendoring shape needs it to. Replacing a tarball **under its existing filename** leaves `pnpm-lock.yaml` recording the previous archive's `integrity`, and pnpm does not notice: a plain install, `--fix-lockfile`, and `--force` all keep the recorded resolution and reinstall the old bytes from the store, so the vendored file and the installed package silently disagree. Evicting the entry is what re-hashes it — point the specifier at another filename, install, point it back, install again — and the proof is the `integrity` line moving to the new archive's `sha512`. Every re-vendoring before `@haoran/dsh-btw` 0.1.0's `private: true` fix carried a new version in the filename, which is why the trap had not been hit; `verify-third-party-notices`, now a `doc-sync` leaf beside this gate, does not cover it either.

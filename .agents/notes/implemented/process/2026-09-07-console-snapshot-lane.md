...
The four experimental plugins the patch names become root `devDependencies`, beside `@deepseek-ai/dsh-tool-session-query` and `@deepseek-ai/dsh-web-fetch-http`, which `snapshots/session/session-query-spill` and `snapshots/session/web-fetch` declare there for the same reason and which appear in no `apps/*` manifest. That is the route the snapshot harness provides: `materializeProfilePatch` hands every bare plugin name to `linkProfilePackage`, whose `packageDirFromPatch` searches `createRequire(pathToFileURL(<the authored patch>)).resolve.paths(name)` — anchored on `snapshots/console/cordis.yml`, so the repository root's `node_modules` is on that chain — and symlinks the hit into the child's `.dsh/profiles/node_modules`. The installation mirror is a different mechanism and cannot carry any of them: `resolveModuleFallbackEntries` walks `profileDependencyNames`, which is `dependencies` plus `peerDependencies` and nothing else. That mirror is what carries `dsh-host-webserver`, which this patch also names and which `packageDirFromPatch` misses, so the two routes serve different rows of the same composition.
...
The byte-identical header pin is the load-bearing result: it is the evidence that a patch over the shipped profile composes the same request header — persona, prompt sections, and tool schemas — that a hand-rolled demo spine did. It is not a claim that nothing model-visible changed: the profile's sandbox rows add one runtime-context paragraph to every session, which is model-visible input arriving as a session message rather than in the header, and Consequences below is where this lane accepts it. Every tool-call argument and every tool-result text survives byte for byte too — `show_component`'s `dataSource` paragraph with its `gridItems`-may-be-omitted sentence and `page.currentPage`, the `show-default-columns-turn` approval card that names no column, its result line ending `Page 1 of 1.`, and `empty-datasource-turn`'s zero-rows sentence all read exactly as they did.
...
**Declare the four plugin names in `apps/cli`'s `devDependencies`.** Rejected. It does work — Node resolves them from the dsh installation's own `node_modules`, in both launch modes — and no rule forbids it: `checkExperimentalDependencyIsolation` ranges over `dependencies`, `optionalDependencies` and `peerDependencies` only, and `apps/cli` already carries four experimental devDependencies. But `apps/cli/package.json` is the published CLI manifest and this fork's conflict surface on every upstream sync, and a snapshot lane is not worth a hunk in it when the root manifest already carries two other lanes' plugins for exactly this purpose.

**Make `snapshots/console/` a pnpm workspace member with its own `package.json`, so the patch's plugin names resolve from a nearer `node_modules`.** Rejected: the root manifest already puts those packages on the patch's own resolution chain, so a closer `node_modules` buys nothing. It would add a second resolution path for one directory and put a package manifest in a corpus tree that has none.
...
`pnpm run test:snapshot snapshots/console`: eleven scenarios and seven fixture guards, all passing, keyless — and the same under `DSH_EXAMPLE_MODE=lib`, which is the mode CI's snapshot gate runs and the one where the tsconfig-paths route is absent, so it is the mode that proves the plugin names resolve from the root manifest. The lane discriminates: changing one word of `show_component`'s description in `packages/experimental/component-surface` fails all eleven, because every classmate's composed request header is compared against the pin, and each failure names the changed sentence. Reverting restores them.

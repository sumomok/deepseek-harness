...
`packages/bundle/base/tests/base.spec.ts` and `packages/bundle/sdk-minimal/tests/sdk-minimal.spec.ts` (both already parsing `cordis.patch.yml` through `entryListSchema` for other rows' `disabled`/`config` shape) gained assertions that both rows parse to `disabled: true` in their respective bundle.

...
`packages/bundle/base/tests/base.spec.ts` 与 `packages/bundle/sdk-minimal/tests/sdk-minimal.spec.ts`(两者本就通过 `entryListSchema` 解析 `cordis.patch.yml` 来检查其他行的 `disabled`/`config` 结构)新增了断言,确认两个 bundle 里对应的行都解析为 `disabled: true`。

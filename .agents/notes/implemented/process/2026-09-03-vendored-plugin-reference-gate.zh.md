...
这里没有任何东西检查 lockfile,而有一种重新 vendor 的形态偏偏需要它。**沿用原有文件名**替换一个 tarball 时,`pnpm-lock.yaml` 里留着的仍是上一份归档的 `integrity`,而 pnpm 不会察觉:普通安装、`--fix-lockfile` 与 `--force` 都保留已记录的解析结果,并从 store 里把旧的字节重新装上,于是 vendor 进来的文件与实际安装的包悄悄不一致。把那条记录逐出去才会重新算哈希——把标识符指到另一个文件名、安装、再指回来、再安装一次——证据是 `integrity` 那一行变成新归档的 `sha512`。在 `@haoran/dsh-btw` 0.1.0 那次补 `private: true` 之前,每一次重新 vendor 都在文件名里带着新版本号,所以这个坑一直没踩到;如今已成为 `doc-sync` 叶子、与本门禁并列的 `verify-third-party-notices` 也覆盖不到它。

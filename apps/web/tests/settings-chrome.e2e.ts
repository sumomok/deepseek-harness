...
describe('web e2e: settings modal and General preferences', () => {
  ...
  it('opens the settings dialog, switches sections, and closes by every path', async () => {
    ...
    await dialog.getByRole('button', { name: '供应商与模型' }).click()
    await expect.poll(() => dialog.getByRole('button', { name: '供应商与模型' }).getAttribute('aria-current'), { timeout: 5_000 }).toBe('true')
    ...
    expect(await dialog.getByRole('button', { name: '供应商与模型' }).getAttribute('aria-current')).toBeNull()
  ...
  it('persists the settings language across reload and a distinct port', async () => {
    ...
    expect(await enDialog.getByRole('button', { name: 'General settings' }).getAttribute('aria-current')).toBe('true')

...
function stableSessionFixture(
  ...
  const harvested = scrubSessionSnapshot(normalizeWebSessionVolatiles(rawSessionLog(session)))
    .split(session.id).join('{{sessionId}}')
    .split(workspaceCwd).join('{{cwd}}')
  const fresh = keep(harvested)
...
export async function recordFixture(
  ...
  const keep = options.afterSeed === true ? withoutSeededHistory : undefined
  await writeFile(fixturePath, stableSessionFixture(agent.session, existing, scaffold.workspaceCwd, keep))
}

...
| `@haoran/dsh-auto-compact` | `0.2.1`,来自提交进本仓库的 tarball | 在对话塞满之前按你选的点位压缩它:设置页「插件」下的一张卡片,带一个开关和一个滑块,滑块读的是 20% 到 95%,量的正是输入框旁边那个用量圆环所报的同一个上下文窗口数字。一个值管所有对话、所有模型。**这个插件的**压缩发生在一段回答答完之后,绝不在一轮的两步之间——harness 自己的溢出兜底仍然会在答的过程中动手,那条没被碰。它的 `compactionPolicy` 服务答 `isEnabled(): false`,把 harness 两步之间的压力路径关掉,然后由插件在 running→idle 的转换上调 `ctx.compaction.compactNow()`:每次 driver 退出只试一次,所以摘要失败时是等下一段回答,而不是每一步都重试一遍。它跳过用户按了停止的那一次、正在被关掉的会话、以及所有子会话;引擎经 `agentPresets.serviceFor` 找,因为每个 preset 都把它装在自己的 realm 里——位子只有在这次查找成功之后才答 `false`,所以 harness 绝不会在后面没人接手的情况下停手。那个入口只把最后一条消息原样留下,压缩幅度就是 `/compact` 的幅度,设置卡上写了这一点。关掉开关只是不再提前压缩,并不影响内核自己的溢出兜底,那是这个插件搬不动的地板 |

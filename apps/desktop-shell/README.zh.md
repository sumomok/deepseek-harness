...
| `@haoran/dsh-auto-compact` | `0.1.0`,来自提交进本仓库的 tarball | 在对话塞满之前按你选的点位压缩它:设置页「插件」下的一张卡片,带一个开关和一个滑块,滑块读的是 20% 到 95%,量的正是输入框旁边那个用量圆环所报的同一个上下文窗口数字。一个值管所有对话、所有模型。插件自己不测量也不压缩——它把开关与百分比作为一个 `compactionPolicy` 服务发布出去,由 agent loop 实时读取,压缩本身由内核自己的 `compaction-basic` 执行。关掉开关只是不再提前压缩,并不影响内核自己的溢出兜底,那是这个插件搬不动的地板 |

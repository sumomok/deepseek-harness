...
| `@haoran/dsh-auto-compact` | `0.2.0`,来自提交进本仓库的 tarball | 在对话塞满之前按你选的点位压缩它:设置页「插件」下的一张卡片,带一个开关和一个滑块,滑块读的是 20% 到 95%,量的正是输入框旁边那个用量圆环所报的同一个上下文窗口数字。一个值管所有对话、所有模型。压缩发生在一轮结束之后,绝不在一轮的两步之间:插件的 `compactionPolicy` 服务答 `isEnabled(): false`,把内核自己那条两步之间的压力路径关掉,然后由插件在 running→idle 的转换上调 `ctx.compaction.compactNow()`——每轮结束只试一次,所以摘要失败时是等下一轮,而不是每一步都重试一遍。那个入口不留原样的尾巴,压缩幅度就是 `/compact` 的幅度。关掉开关只是不再提前压缩,并不影响内核自己的溢出兜底,那是这个插件搬不动的地板 |

...
| `@haoran/dsh-auto-compact` | `0.2.2`,来自提交进本仓库的 tarball | 在对话塞满之前按你选的点位压缩它:设置页「插件」下的一张卡片,带一个开关和一个滑块,滑块读的是 20% 到 95%,量的正是输入框旁边那个用量圆环所报的同一个上下文窗口数字。一个值管所有对话、所有模型。**这个插件的**压缩发生在一段回答答完之后,绝不在一轮的两步之间;harness 自己的溢出兜底仍然会在答的过程中动手,那是这个插件开也好关也好都搬不动的地板。它的 `compactionPolicy` 服务答 `isEnabled(): false`,把 harness 两步之间的压力路径关掉,然后由插件在会话空下来时调 `ctx.compaction.compactNow()`:每次 driver 退出一次——也就是一段回答连同排在它后面的消息共享的那一段——所以摘要失败时是等下一段回答,而不是每一步都重试一遍。它跳过用户按了停止的那一次和正在被关掉的会话;子会话和别的会话一样会被压,因为同一个 `isEnabled(): false` 把它们两步之间的路也关了。引擎按会话经 `agentPresets.serviceFor` 找,因为每个 preset 都把它装在自己的 realm 里,位子也只有在这次查找在某处成功之后才答 `false`。它把最后一条消息原样留下——为保持工具调用成对可能多留几条——其余压成一条摘要,这正是 `/compact` 的幅度,设置卡上也这么写。压缩进行时界面上不显示任何东西,这期间发出的消息会等它结束后才发出 |

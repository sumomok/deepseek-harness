 ...
 * `apps/desktop-shell/electron-builder.yml` declares — no `artifactName` overrides
 * it — so they follow from `productName` and each target's single arch.
 * `apps/desktop-shell/tests/artifact-names.spec.ts` reads that config and pins both
 * against it.
 * @module
 */

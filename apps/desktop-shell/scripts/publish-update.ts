 ...
 *
 * Usage: pnpm exec tsx apps/desktop-shell/scripts/publish-update.ts --notes <file>
 ...
 * @param version - the version apps/desktop-shell/package.json declares.
 * @returns the parsed manifest.
 */
async function readManifest(channel: Channel, version: string): Promise<Manifest> {
  const path = join(DIST, channel.manifest)
  if (!existsSync(path)) {
    throw new Error(`publish: ${path} is missing — build first: pnpm exec tsx apps/desktop-shell/scripts/package.ts --mac --win`)
  }
...
async function main(): Promise<void> {
  ...
  if (live !== undefined) {
    ...
    if (order < 0) {
      throw new Error(`publish: ${version} is older than the published ${live.version}; bump apps/desktop-shell/package.json first.`)
    }
    ...
    if (order === 0 && !cli.republish) {
      throw new Error(`publish: the feed already serves ${version}; pass --republish to overwrite it (repairing a partial upload), or bump apps/desktop-shell/package.json.`)
    }
  }

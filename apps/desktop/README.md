...
**If you installed one of these plugins yourself before this build**, the profile's own `node_modules` still holds that copy, and the Loader finds it before the shipped one while the patch layer keeps coming from the payload. The launch says so — `warning: profile copy dsh-at-file@0.6.3 shadows the shipped 0.6.5 module` — and changes nothing, because the profile's dependencies belong to whoever installed them. `dsh plugin --profile web remove <name>` drops the profile's copy and leaves the shipped one, which is the state a fresh install has.

**To turn one off, disable its row** in `$DSH_HOME/profiles/web/cordis.patch.yml` — `dsh-at-file` for the mentions, `better-sidebar` for the sidebar, `screenshot` for the capture tool:
...
| `504` | The request passed its 30-second deadline |

Every failure body is one line of `text/plain`, because its reader is a tool that quotes it into the message the model sees.

**Each render gets a hidden window that shares nothing with the app's own.** Its session has no `persist:` prefix, so it lives in memory and dies with the window: a rendered page cannot read or write the cookies, storage, or caches of the window the user is working in, and nothing it stores outlives the request. There is no Node integration, no `webview`, no devtools; every permission request is denied, and every download and every window the page tries to open is refused. The window is destroyed on the reply, on a load failure, and on the deadline.

**What bounds it**: one render at a time, four accepted requests at once (one rendering and three waiting), and a 30-second deadline measured from acceptance rather than from the start of the render — so a request that spent the window queued is answered 504 without ever opening a window. A `fullPage` capture measures `document.documentElement.scrollHeight` and resizes to it, clamped at 8192 px, because a document with an infinite scroller reports a height that grows while it is measured.

**Nothing else on the machine or off it can reach the service.** The listener binds loopback, so nothing off the machine can; the token is compared in constant time, so another local process cannot use it by finding the port; and no CORS header is ever sent while every method other than `POST /render` answers 404, so the preflight that an `authorization` header and a JSON content type force a browser to send is refused — which is what keeps a page in the user's own browser out.
...
- The render service is per-launch and serial. Four requests are accepted at a time and one renders, so a page that takes the whole 30-second deadline to load holds the slot, and requests queued behind it can be answered 504 without ever opening a window.

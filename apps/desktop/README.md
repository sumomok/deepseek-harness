...
Builds are unsigned development artifacts: macOS Gatekeeper needs right-click-open (or `xattr -dr com.apple.quarantine`) on machines other than the builder's, and Windows SmartScreen shows its unknown-publisher prompt. A Windows package cross-built from macOS is structurally verified only — smoke-test it on a real Windows machine before handing it to anyone; building on Windows is what lets that smoke test run against the package just built.

...
| `@haoran/dsh-default-model` | `0.2.0`,来自提交进本仓库的 tarball | 出厂默认模型:全新安装的第一个会话开在 `deepseek-flash`(DeepSeek V4.1 Flash)上,选择器把它列为 `default`。已经自己选过模型的用户仍保留那个选择,它在他们自己的设置里 |

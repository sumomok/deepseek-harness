...
| `@haoran/dsh-default-model` | `0.3.0`,来自提交进本仓库的 tarball | 出厂默认模型,也是选择器里唯一的模型:`deepseek-flash`——DeepSeek-V4.1-Flash,文本与图片都收。DeepSeek 已下线 V4 Flash 系列并把 V4 Pro 的请求转到它,所以目录只列这一个。在此之前选过模型的用户仍保留那个选择,它在他们自己的设置里;若那是一个已下线的模型,文本照常,图片会被拒,直到改选 DeepSeek-V4.1-Flash |

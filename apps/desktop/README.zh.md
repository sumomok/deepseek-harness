...
**那份清单里的最后一个 bundle 是本产品自己的组合层。**`@deepseek-ai/dsh-desktop-app`（[apps/desktop-app](../desktop-app/README.zh.md)）不含代码,也不是插件:它就是一份 `cordis.patch.yml`,装着出厂的 `dsh-base` 与 `dsh-web-app` 留给部署方去定的那些行。把它排在最后,是因为已有 profile 缺的名字是往后追加的,末位是全新 profile 与升级而来的 profile 都会给它的唯一位置;这个位置也让这里的一行盖过其上的每一层,而 `$DSH_HOME/profiles/desktop/cordis.patch.yml` 仍在它之后生效。它今天携带的唯一一行打开了对话正文的全文搜索:出厂组合包把 `session-query-sqlite` 的 `openAt` 设为 `never`,所以侧栏搜索只匹配会话标题与工作区名;本层把它设为 `first-search`,索引落在 `~/.dsh/session-search/desktop.db` 这份持久文件上。启动时什么都不打开;一次运行里的首次搜索负责建立或对账索引,之后每次搜索只读发生过变化的部分。想回到只搜标题,就在你自己的 patch 层里把整行重述一遍并写上 `openAt: never`——以 id 为目标的 patch 会替换整个 `config`,所以那次重述必须把 `path` 一并带上。

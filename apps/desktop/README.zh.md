...
| `@sumomok/dsh-balance` | `0.4.2`,来自提交进本仓库的 tarball | 账户余额与花掉了多少:侧栏底部一个显示供应商那边剩余额度的 chip、输入框下方的本会话成本行,以及按本部署自己维护的价格表算出的今日 / 本月 / 累计花费,默认表里带着 DeepSeek 的 CNY 与 USD 价格,含 2026-09-10 生效的 Flash 系列新价。chip 的浮层里带一个**充值**按钮,对插件收录了控制台页面的那些供应商可见,点开走系统浏览器。设置页的**余额**里有一个开关,把 chip 上的数字换成圆点,给正在共享或录制的屏幕用;它在保存那一刻就到达 chip,不用等下一次轮询;浮层、余额偏低的染色与输入框下方的本会话成本行都照旧显示 |

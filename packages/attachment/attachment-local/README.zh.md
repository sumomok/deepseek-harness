...
文本文件每条消息最多准入 10 个文件、UTF-8 字节总量不超过 10MiB；每个文件最多 1MiB。准入会拒绝文件中任何位置的 NUL 字节或非法 UTF-8（`NOT_TEXT_FILE`），以及经路径与控制字符剥离后仍不可用的显示名（`INVALID_FILE_NAME`）；它按原样存储提交的字节——与图片不同，文本没有规范化或请求投影形式。文本对象与图片共用同一个内容寻址的 `objects/` 目录树：两种附件都是按摘要寻址的二进制对象，仅引用本身——图片元数据与显示名——因附件种类而异。

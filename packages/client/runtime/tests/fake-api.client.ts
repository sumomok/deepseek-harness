...
export class FakeApiClient implements IApiClient {
  ...
  onFile: (payload: unknown) => Promise<RpcResponse<{ attachment: { attachmentId: never; name: string; bytes: number }; text: string }>> =
    () => Promise.resolve(ok({ attachment: { attachmentId: 'f' as never, name: 'a.txt', bytes: 1 }, text: 'a' }))
  ...
  onProbeTargets: (payload: unknown) => Promise<RpcResponse<{
    results: { path: string; exists: boolean; kind?: 'file' | 'dir' }[]
  }>> =
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    file: (payload: unknown) => this.record('session.file', payload, this.onFile(payload)),
  ...
  readonly host: IApiClient['host'] = {
    ...
    probeTargets: (payload: unknown) => this.record('host.probeTargets', payload, this.onProbeTargets(payload)),
  }

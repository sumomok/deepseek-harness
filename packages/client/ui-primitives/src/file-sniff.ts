 ...
 * one that can reject a submission. Lives here, not in the composer package
 * that first needed it, because both the composer (`ui-conversation`) and
 * the attachment presentation plugin (`ui-attachment`) call it from their
 * own document-level drop/paste listeners, and a dynamic `dsh.client`
 * feature package cannot import another one's runtime values.
 *
 * @module @deepseek-ai/dsh-client-ui-primitives/file-sniff
 */

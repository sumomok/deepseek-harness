...
对话 UI 的动态附件呈现插件。它通过 `ctx.slots.inject` 等待 conversation 包声明 `conversation.input.attachments` 与 `conversation.message.images`，随后注册输入框草稿图片栏、草稿文件条列表、文档拖放目标、聊天历史图片画廊和原图灯箱。conversation slot 持有方提供附件数据、图片加载、回调及其命名空间翻译器；呈现组件保持纯 props，且不从包入口导出。
...
`FileChipRow` 将待发送草稿文本文件渲染为条形卡片，在附件栏旁以可换行的行排列，与附件栏共用同一左侧内边距，两者同时存在时条形卡片行位于附件栏下方 6px 处——文件没有缩略图可展示，因此不复用附件栏固定 64px 的条目形态，而是拥有自己的形态。每个条形卡片显示一个小型文档图形、文件名（超长省略号截断，完整名称落在 `title` 上）与字节大小文案，删除按钮内联排列（不做绝对定位、不预留死区），在行内占据自己的位置，悬停或键盘聚焦时显示，与附件栏缩略图一致（粗指针设备常显）。条形卡片本身没有打开动作：已发送文件的默认打开行为是消息气泡上 referent/open 缝隙驱动的内联展开/折叠查看器，而非草稿态预览。草稿命中发送前密钥容器启发式判定（`secretContainerHitIds`）的条形卡片还会渲染描边、警示色圆点与一段简短的行内文字标签——文案不会暗示曾读取内容，只说明这类文件通常存放密钥；只要有任意一张卡片命中，行下方还会渲染一条提示文字，指出第一个命中的文件名，并带有自己的删除按钮。是否挂载由持有方决定，仅在存在文件草稿时渲染。
...
- **已发送文件尚无历史渲染** — `MessageImage`/`ImageGallery` 通过 `conversation.message.images` 渲染已发送图片；已发送文件的卡片与内联展开/折叠查看器直接落在对话消息气泡上，不经由本包填充的 slot，目前尚未实现。

...
- **The rendered column is not covered by an assembled snapshot** — this package's on-display prompt section is pinned by the [`examples/content-console`](../../../examples/content-console/README.md) lane, which composes this row for real; what the column draws from an entry is browser evidence, and that is a Playwright scenario against a real composition.

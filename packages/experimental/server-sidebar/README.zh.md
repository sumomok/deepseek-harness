...
工作台是本外壳始终落位的那一个持久默认对话。点击它，或者在没有当前选中会话的情况下加载侧边栏，都会解析已记录的 `workbenchSessionId` 并在其仍然存活时重新打开；若尚无记录（首次使用）或已记录的那个已经不在，则针对最近使用的工作区创建一个新会话，并记录新的 id。`workbenchSessionId` 是弱引用，理由与下文一条工作流的 `homeSessionId`完全相同：这里没有任何东西拥有会话删除的所有权，一个失效的指针会降级为一个全新对话，而不会污染指名它的那份文档。
...
- **改名／移除／重新排序**——用悬停显现的图标按钮（沿用原收藏菜单自己的交互习惯），而非原生右键菜单；用上移/下移按钮，而非 HTML5 拖拽——两者都是任务本身明确允许的降级条款下的 v1 降级；见「已知限制」。重新排序跟随用户拖拽顺序（决策⑤），在这里表现为对 `order` 字段的显式上移/下移交换。
...
- **重新排序是上移/下移按钮，而非拖拽；改名/移除/重新排序用悬停显现的图标按钮，而非原生右键菜单。** 两者都是任务本身明确允许的 v1 降级（「若实现体量失控，降级为右键菜单「上移/下移」」），考虑到本次改动的整体体量而在此处使用。
- **`ui-workspace` 是不被组合，而非仅仅被隐藏。** 一次零工作区的全新安装,依然会让页面或工作流点击成为一次被吸收的空操作（见上文「导航」）——这是从此前基于收藏的设计里延续下来的、已经被接受的既有边界情况，并非本次新引入。

...
| `/permission` | *（不存在可禁用的行）* | `@deepseek-ai/dsh-permission-presets` 是在 `ctx.inject(['commands'], …)` 里注册这条命令的，而这个注册表还有本控制台组合的另外四类消费者，因此 overlay 改为只在那一行上隔离这个名字——见下文。 |
...
| 输入框拒绝输入时的占位文案 | `[data-composer-input][aria-disabled] + [data-composer-placeholder]` 及其 `::after` | 与上一行互补的同两个属性 | 同上 |
...
输入框占位文案用的是与英雄区标题相同的 `::after` 手法，但需要两条规则，因为 `InputBar.tsx` 为一条比看上去更长的占位文案阶梯只渲染同一个 `[data-composer-placeholder]` 元素（`InputBar.tsx:467-476`：先是拥有方传入的 prop——英雄态文案、不可用输入框的说明、被抬起的阻断态的理由——然后是 `parentOffline`，然后是其余任何 disabled 状态，然后是排队引导提示，然后是 plan 模式，最后才是默认文案）。这两条规则按「输入框此刻能做什么」把它切开。**可接受输入**读作「说说要做什么」，也就是那句邀请。**有会话但拒绝输入**读作「暂时无法输入」——`placeholder.unavailable`（会话不可用 /「Session unavailable」）与 `placeholder.parentOffline`（父会话已离线…）都落在这里，这是决策②再无其他手段够得到的禁用会话词汇，因为两者都属于 `ui-conversation` 自己的 locale 命名空间；这次替换保留了访客需要知道的事实，去掉了那个词。**没有连接工作区的不可用输入框**则原样不动：它的占位文字是唯一在说明输入框为何不可用的东西（见「已知限制」）。`data-phase` 就是 `input?.phase ?? 'inert'`（`InputBar.tsx:533`），`aria-disabled` 就是 `editorDisabled`（`:534`）——每一种拒绝输入的状态都会设置它，而工作区选择触发器有意不设，正是这处不对称让这个三分法写得出来。替换后的文案带的是 `ui-conversation` 自己的字号 token，而不是一个写死的像素值，因此它继续跟随设置里的字号偏好。别的路都没有：`conversation.composer.bar` 的 `placeholder` prop 只有 `ConversationRoot` 会提供，而 locale 注册表会拒绝对它已经持有的 namespace/locale 对的第二次注册。兄弟节点 `[data-composer-input]` 上的 `data-placeholder` 与 `aria-label` 原样不动——见「已知限制」。
...
本插件不属于任何出厂 bundle。`overlay/customer.patch.yml` 就是完整的客户表单 overlay：它禁用 `ui-layout`、`ui-sidebar`、`ui-agent-preset`、`ui-brand-official`、`ui-cordis`、`ui-trajectory`、`ui-model-selection`、`session-log-download`、`ui-settings-models`、`ui-permission`，并插入 `server-layout`、`content-surface`、`content-column`、本包，以及 `library-skills` 行（一个以 `@deepseek-ai/dsh-experimental-library-skills` 出厂 `skills/` 为根的隔离 `skill-filesystem` provider）。它还重新配置（而不是禁用）了一个出厂行。`permission` 行做三件事：把预设表改写为面向客户的名字——出厂的 `workspace-write` 名字正是输入框权限 chip 显示的那一个，而这个 chip 在宿主给出的名字与内置默认不同时会原样透传；写明 settings 文档里没有存值时新会话被钉住的 `defaultPreset`（见上文「去术语化」）；以及携带 `isolate: { commands: true }`，正是它让 `/permission` 从未注册（见上文「去术语化」）。`packages/experimental/server-sidebar/tests/customer-overlay.client.spec.ts` 钉住这三件事，并钉住 `apps/web/tests/` 下那三份控制台 overlay 仍然携带同一行。它不插入 `content-frame`，也不插入 `component-surface`——部署自己的页面目录与视图目录需要单独组合，与它并列，因为两者各自携带这份 overlay 无法预设的部署专属配置。它同样不插入 [`auth-gate`](../auth-gate/README.zh.md)，而底部那个退出按钮正需要这一行：没有组合它时按钮照样渲染，但按下去只会向控制台报告登录页未知，然后停在原地。两份 overlay 都携带本包那一个必填的 `config` 字段（见上文「配置」）；缺了它的行会在加载期失败。用 `dsh --profile <name> --patch <path>` 应用；overlay 点名的每一个包都必须能从 profile 目录解析到——对树外插件而言即 `dsh plugin --profile <name> add <path>` 或等价的链接；发布 bundle 不得声明实验性包。需要这样安装的一共五个包：`dsh-experimental-server-layout`、`dsh-experimental-content-surface`、`dsh-experimental-content-column`、本包，以及 `dsh-experimental-library-skills`。最后一个是被某一行的 `bundledSkillDir` 表达式点名的，而不是被某一行的插件名点名，但它一样是必需的：profile 里缺了它，该行加载时就抛 `MODULE_NOT_FOUND`，entry tree 永远无法就绪，**控制台起不来**。
...
- **拒绝输入时输入框自己的理由是被替换掉的，不是被翻译。** `placeholder.unavailable`（会话不可用 /「Session unavailable」）与 `placeholder.parentOffline` 是 `dsh-client-ui-conversation` 为「输入 face 缺席、会话已被移出、或可续的父会话已离线」准备的文案。三者都能被走到——第一种在每一次打开或切换对话、输入 face 还没到达的那个窗口里就会出现——而三者都带着决策②的禁用词汇，且位于任何插件都无法注册进去的命名空间，因此守卫把渲染出来的文字换成「暂时无法输入」，而不是把那个词留在屏幕上。代价是三者之间的区别没了：无论哪一种，访客看到的都是同一句话；而被抬起的输入阻断态——在这份组合里是死的，因为 `ui-model-selection` 已被禁用——一旦某次部署把它带回来，它自己的理由也会被同样地盖住。上游的 `data-placeholder` 与 `aria-label` 仍然携带原句，与上面那条邀请文案一样。

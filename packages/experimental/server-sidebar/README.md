...
- **The composer's access-preset control names a Workspace.** `dsh-client-ui-conversation`'s own `PermissionSelect` labels one preset "Workspace Write", which decision ② does not cover: unlike the surfaces above it is a permission control, not vocabulary chrome, so hiding it would take away a choice rather than rename one. Removing that word needs a decision about the control itself, not another `terminology-guard.ts` rule.

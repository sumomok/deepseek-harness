 ...
 * assertion observes the served HTTP surface — the favorites document, the
 * same-site and content-type fences on the mutating method, the schema and
 * duplicate-session refusals, persistence surviving a reload of the
 * composition, and route release on fiber disposal.
 *
...
import { SERVER_MENU_FAVORITES_ROUTE } from '../src/route.ts'
...
/** POST one favorites document. */
function postFavorites(ctx: Context, body: unknown, headers: Record<string, string> = {}): Promise<Answer> {
  return call(ctx, SERVER_MENU_FAVORITES_ROUTE, {
    method: 'POST',
...
describe('server-sidebar favorites route', () => {
  it('answers an empty list before anything is saved, uncached', async () => {
    const ctx = await loadComposition()
    const answer = await call(ctx, SERVER_MENU_FAVORITES_ROUTE)
    expect(answer.status).toBe(200)
    expect(answer.type).toBe('application/json')
    expect(answer.cacheControl).toBe('no-store')
    expect(JSON.parse(answer.body)).toEqual({ favorites: [] })
  })

  it('serves a HEAD of the favorites document', async () => {
    const ctx = await loadComposition()
    expect((await call(ctx, SERVER_MENU_FAVORITES_ROUTE, { method: 'HEAD' })).status).toBe(200)
  })

  it('persists a posted list and answers the server\'s authoritative copy', async () => {
    const ctx = await loadComposition()
    const favorites = [{ sessionId: 's1', label: 'Alpha', order: 0 }, { sessionId: 's2', label: 'Beta', order: 1 }]
    const posted = await postFavorites(ctx, { favorites })
    expect(posted.status).toBe(200)
    expect(JSON.parse(posted.body)).toEqual({ favorites })

    const read = await call(ctx, SERVER_MENU_FAVORITES_ROUTE)
    expect(JSON.parse(read.body)).toEqual({ favorites })
  })

  it('refuses a favorites list with a duplicate session id', async () => {
    const ctx = await loadComposition()
    const answer = await postFavorites(ctx, {
      favorites: [{ sessionId: 's1', label: 'A', order: 0 }, { sessionId: 's1', label: 'B', order: 1 }],
    })
    expect(answer.status).toBe(400)
    expect(JSON.parse(answer.body)).toEqual({ error: 'server-sidebar: duplicate favorite for session "s1"' })
    expect(JSON.parse((await call(ctx, SERVER_MENU_FAVORITES_ROUTE)).body)).toEqual({ favorites: [] })
  })

  it('refuses a body shaped wrong before it ever reaches the schema', async () => {
    const ctx = await loadComposition()
    for (const body of ['not json', {}, { favorites: 'nope' }, { favorites: [{ sessionId: 1 }] }]) {
      const answer = await postFavorites(ctx, body)
      expect(answer.status).toBe(400)
    }
  })
  ...
  it('refuses a post a browser labelled cross-site', async () => {
    const ctx = await loadComposition()
    const answer = await postFavorites(ctx, { favorites: [] }, { 'sec-fetch-site': 'cross-site' })
    expect(answer.status).toBe(403)
    expect(JSON.parse(answer.body)).toEqual({ error: 'server-sidebar: the favorites route serves same-site requests only' })
  })
  ...
  it('refuses a post that is not sent as JSON', async () => {
    ...
    expect(JSON.parse(answer.body)).toEqual({ error: 'server-sidebar: the favorites route accepts application/json only' })
  })

  it('refuses a body past the bound instead of buffering it', async () => {
    const ctx = await loadComposition()
    const answer = await postFavorites(ctx, {
      favorites: [{ sessionId: 's1', label: 'A'.repeat(80 * 1024), order: 0 }],
    })
  ...
  it('states the complete method set it serves', async () => {
    const ctx = await loadComposition()
    const answer = await call(ctx, SERVER_MENU_FAVORITES_ROUTE, { method: 'DELETE' })
  ...
  it('persists across a reload of the composition (per-account durability)', async () => {
    const ctx = await loadComposition()
    const world1 = world
    await postFavorites(ctx, { favorites: [{ sessionId: 's1', label: 'Alpha', order: 0 }] })
    await ctx.fiber.dispose()
    context = undefined

    const reloaded = await loadComposition(world1)
    expect(JSON.parse((await call(reloaded, SERVER_MENU_FAVORITES_ROUTE)).body))
      .toEqual({ favorites: [{ sessionId: 's1', label: 'Alpha', order: 0 }] })
  })
  ...
  it('releases the route when the fiber disposes (HMR safety)', async () => {
    ...
    const answer = await fetch(`http://127.0.0.1:${String(ctx.webServer.port)}${SERVER_MENU_FAVORITES_ROUTE}`)
...
describe('server-sidebar without the settings capability', () => {
  it('loads with no favorites route rather than failing the row', async () => {
    ...
    const answer = await fetch(`http://127.0.0.1:${String(context.webServer.port)}${SERVER_MENU_FAVORITES_ROUTE}`)
    expect(answer.status).toBe(404)
    await answer.arrayBuffer()
  })
})

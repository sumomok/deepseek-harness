 ...
 *
 * A byte-identical rule to `dsh-experimental-content-frame`'s own copy
 * (installed under a distinct `STYLE_ID` so the two packages' effects never
 * collide when both are composed): the selector hides an empty commandview
 * row regardless of which command left it empty.
 * @module @deepseek-ai/dsh-experimental-content-column/client/hide-empty-command-row
 */

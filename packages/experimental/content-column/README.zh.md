...
座位之上是一条切换条，按最新在前列出该会话的 entry，每个标签页只显示该 entry 的 title——kind 是这一栏把 entry 派发到座位的依据，不是拿给用户看的词。选择是纯 UI 行为：选项存在按会话 id 索引的组件本地状态里，默认落在最新一条，所指 entry 被替换时回落到最新一条，且永不进入会话日志。什么都没产生过的会话得到空状态提示，没有当前会话的浏览器也一样。

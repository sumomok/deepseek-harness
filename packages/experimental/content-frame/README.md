...
- **Only the last `cacheSize` (session, page) pairs are readable** — a console answers for a session behind the one on display through the frame still holding that session's page, and that frame is the first thing evicted when the bound is reached. A session whose page this console never showed, or whose frame is gone, ends its read on the host's claim window instead.

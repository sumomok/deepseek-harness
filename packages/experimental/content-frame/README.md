...
- **The one-card mark is per process** — a user who declines is not asked again in that session, but the mark lives in memory, so a reloaded session or a restarted host asks once more. A card that ended in anything other than a decision — a dropped connection, an answerer that threw — leaves no mark at all, so the model's next attempt raises another card.

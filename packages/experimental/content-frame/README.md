...
- **The row carries the element's class tokens where its name would go.** `e17 clickable {{class: el-tooltip operation-modify el-icon-edit}}` — at most four tokens, in the order the element carries them, an ellipsis where there are more, and nothing at all where the element carries no class. It is printed for the things a page offers to act on and names nowhere — a click target, a button, a link, a box to fill — and for nothing else: a heading or a region says what it is in its role.
- **Nothing is read out of them.** The row is still named nothing: a step naming it passes `label: ""`, and the seat checks that the page names it nothing still. What `el-icon-edit` means is for a skill about that application to say, and this package never guesses.

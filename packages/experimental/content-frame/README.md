...
`src/client/access/` reads the document in the frame as numbered structure — the regions, the controls, and the shape of each table — for a model that cannot see it. The rules it is written by, and what each of them gives up, are recorded in [the reading Agent Note](../../../.agents/notes/implemented/feature/2026-09-02-content-snapshot-engine.md). Two of them decide what a console table reads as:
...
- **A cell offers what the page makes clickable in it.** The icon a table draws for editing a row often carries no role and no name of any kind; read as text it is nothing at all, so the cell names it `clickable` and gives it a ref, the same way the walk offers such an icon anywhere else on the page.
- **A field is named by the words the page draws in front of it.** A form that ties no label to its field is named by the last thing drawn before it that prints no row of its own, inside the smallest element holding both, as far out as the region the field stands in; those words then print once, as the field's name. Anything else the reader can act on between the two ends the search.
- **A field the page asks for says so.** `(required)` follows a field the page marks with `required` or `aria-required`, and one whose label the page draws a star in front of — the mark a form draws with a stylesheet rather than writing it in the document. `drawnAround` injects that reading (`::before` and `::after`), defaulting to the computed styles.
...
- **The words naming a field must be drawn in an element of their own** — a bare run of text in the same element as the field leaves it unnamed, and a field taking its name from words drawn inside a longer run keeps that run as a row of its own, so those words reach the reader twice. The star is read on the words a field takes its name from and nowhere else, so a field the page names itself and stars on a label it ties to nothing is not reported as required.

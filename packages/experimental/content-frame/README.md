...
Every route this package registers is registered at the server root, and **every request the browser makes for one is resolved against the page's own base before it is sent** — `clientUrl` from `@deepseek-ai/dsh-client-connection/client`, which this package declares as a `dsh.client.external`. That is what makes a deployment published under a path prefix work without changing a route: the reverse proxy strips the prefix on the way in, and the page puts it back on the way out, so `/content-frame/settings` and the three read routes the channel below runs on are asked for as `/console/content-frame/settings`, `/console/content-frame/claim`, and so on. A route posted as the constant spells it would address the origin root instead — an address the deployment's proxy routes nowhere near this process — and the read channel would go silent with the console in front of the user. The one browser address not resolved this way is a page's configured `url`, which the frame requests as it stands; [Pages the agent may show](#pages-the-agent-may-show) says why, and why a prefixed deployment writes the prefix into that value itself.
...
`pages` is the deployment's whole vocabulary for the column, and at least one entry is required — `content_show` exists to choose among them. Each page declares an `id` the agent passes, a `title` the user reads, a `description` written in the agent's terms (it becomes the catalogue line in the tool description), and a same-origin `url`. A URL that names a scheme or a host fails the row at load: the frame carries the shell's authority, so it may only address the dsh origin. That `url` is what the browser requests as it stands, so a deployment served under a path prefix writes the prefix into it (`/console/content-app/`); the `/content-app` route the node half claims does not change, because the reverse proxy strips the prefix before the request reaches this process. The two read like one path in an overlay and are not: one is browser-side, the other process-side.
...
The reading half lives in the page seat because that is the only placement holding the frame elements. Visibility and geometry are asked of each element's own window rather than the top one — a frame's layout belongs to that frame. A tab the user is not looking at claims all the same: it holds the same mounted frames and the same live documents, and a window another window covers, a locked screen and a tab in the background all report `hidden` with the console open in front of the user. Visibility orders the bidding instead of gating it — a seat whose tab is not in front waits 250ms before its first bid, so a tab that is in front takes the call wherever there is one — and every seat forgets the calls it gave up on either way, or the call it lost would be unclaimable for the rest of its life. Nor is the session on display a filter: one console answers for every session whose page it still holds a frame of, because switching sessions hides a frame rather than unmounting it, and the read runs against the page that session last had in front. A session this console never showed a page of, or whose frame `cacheSize` has since evicted, is one it does not bid for at all.
...
A deployment passing that file also sets `pageAccess.actApproval: judged` and `pageAccess.judgedBy: llm-permission-gateway` on its own content-frame row, in the column overlay it already writes. Without it the gate and this row both decide the same call, so a set of steps the gate allowed still puts a card up. The setting is not in the gate overlay for the same reason that file restates `provider` and `model`: a patch replaces the whole `config` of the row it targets, so a second row for this package would have to restate `root`, `pages` and every other field the deployment configured. `overlay/content-column.patch.yml` stays at the default, because a deployment may compose the column with no gate at all and this row asking is the answer that is right without one.
...
- **The settings route assumes an HTTP carrier** — the browser half fetches `/content-frame/settings` relative to the page's own base. A transport that serves the shell without exposing the harness over HTTP would fail the row, the same way the iframe's own route would.
...
- **A hidden tab's clocks are the browser's, and past about five minutes they are one wake-up a minute** — a tab the user is not looking at aligns `setTimeout` to one second at once, and Chrome's intensive throttling then stretches it to roughly a minute. Measured on a real console after 8.4 minutes hidden, with the console's own WebSocket connected throughout: holding a live connection does not exempt the page. Every wait this seat spends is one of those timers — the interval between bids, the grace a tab that is not in front pays before its first bid, the quiet window a page is watched through, the poll between a step's checks, the poll that asks the frame in front where it is — so a read taken in a long-hidden tab can end at the host's report deadline and answer that the console claimed the read and did not answer in time. A step run's own deadline is wall-clock and is not stretched.
- **A canvas in a hidden tab may export a frame the user never saw** — a page that draws through `requestAnimationFrame` stops drawing when its tab goes away, so a picture read exports whatever was painted last, or nothing. Like the WebGL case below it, nothing here can tell that from a page that drew what it drew.
...
- **Only the last `cacheSize` (session, page) pairs are readable** — a console answers for a session behind the one on display through the newest of that session's cached pages the column still shows, so an entry retired or dismissed since falls back to the older page still mounted under it, while a frame the bound evicted does not come back. A session with no such page at all — never shown in this console, or every frame of it evicted — ends its read on the host's claim window instead.

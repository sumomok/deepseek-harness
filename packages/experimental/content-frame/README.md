...
**A picture too small to read is enlarged, each kind its own way.** The provider scales a small image up before it prices one (`MIN_PIXELS`, 384 × 384, in `packages/llm/llm-deepseek/src/image-tokens.ts`) and charges the same on both sides of that floor, so reaching it in the browser is free — and it is what makes a small picture readable, because the enlargement the provider does before pricing is not the picture the model reads. A vector is rasterized to the floor's own area, its ratio being a layout box rather than a stored grid. A bitmap is drawn at the whole multiple of itself that carries its short side there, with smoothing off, so every stored pixel becomes a square block and no edge lands between two of them; every other draw keeps smoothing on, since a picture scaled down needs its dropped pixels averaged rather than discarded. Both are then held to the request's own pixel budget — 640,000, `DEFAULT_REQUEST_IMAGE_PIXEL_BUDGET` — by the same geometry the attachment layer projects a request image with, which is also what walks a strip back down when its whole multiple runs past it.

...
- **The icon rule is a class-name heuristic with two blind spots** — an icon font that never writes the word `icon` reaches the read as nothing, and an icon drawn outside a cell, a toolbar, or a list is read as decoration. Both are the price of a rule the page's own markup, rather than a specification, has to earn.

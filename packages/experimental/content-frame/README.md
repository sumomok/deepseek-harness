...
- **A control the page names nowhere cannot be acted on** — the model can only copy a name the listing printed, and the seat checks what it copied against the page. A box is named by its label, its `aria-label`, or the word written inside it (`placeholder`, `aria-placeholder`); a control with none of those prints no name, and there is nothing for a step to carry.
...
- **What the page did on its own is a heuristic, and a bounded one** — text is reported as a message when it becomes readable while the steps run, at most eight things per call, each cut to 200 characters, over at most thirty-two nodes followed at once. An application that redraws a list mid-call spends that budget on its own churn, and a page that restyles something it was already showing spends a line on text the closing read shows anyway: what a class or an inline style drew before it changed cannot be read back, and missing every framework's own reveal is the worse half of that trade.
- **A call is scoped to the documents it started with** — the stand-ins, the message watch, the `wait` step and the settle wait all cover the same-origin documents the reader walked when the call began. A frame the page adds while the steps run is in none of them: what it draws is left to the closing snapshot, and a dialog it opens blocks the tab as it would have without this package.

...
`pages` 是这一栏在部署里的全部词汇，且至少要有一项——`content_show` 存在的意义就是在其中挑选。每个页面声明 agent 传入的 `id`、用户读到的 `title`、以 agent 的语汇写成的 `description`（它会成为工具描述里的清单行），以及同源的 `url`。带协议或主机名的 URL 会让该行在加载时失败：这个 frame 携带外壳权限，因此只能寻址 dsh 同源地址。这个 `url` 会被浏览器原样请求，因此挂在路径前缀下的部署要把前缀写进它里面（`/console/content-app/`）；而 node 半边认领的 `/content-app` 路由不变，因为反向代理在请求抵达本进程之前就把前缀剥掉了。这两者在组合层里看着像同一条路径，其实不是：一条是浏览器侧的，一条是进程侧的。
...
**很小的图会被放大，放到不再免费为止。** 供应商按它投射出来的那张网格计价，而它会先把任何小于自己那道下限——`MIN_PIXELS`，384 × 384 总像素，见 `packages/llm/llm-deepseek/src/image-tokens.ts`——的图，放大到正好那个面积。于是同一比例、面积都在下限之内的两张图落在同一张网格上，token 数相同；越过下限的图则按自己的像素计价。方图在下限及以下是 117 tokens，面积到下限两倍是 201，占满整个像素预算是 349，对着供应商 384 tokens 的上限；换一个比例，这三处各是别的数，因为价钱来自那张网格而不是面积。所以导出朝这道下限的面积放大，且绝不越过它。矢量栅格化到这个面积，比例按它的布局盒。位图则按仍能装进这个面积的最大整数倍绘制——一枚 32 × 32 的标识放十二倍，一枚 100 × 100 的图标放三倍，而超过下限四分之一的一律一倍、原样不动——并关掉平滑，于是每个存下来的像素成为一个方块，没有一条边落在两个像素之间。这个整数倍就是放大的全部：下限的面积只是请求自身像素预算（640,000，`DEFAULT_REQUEST_IMAGE_PIXEL_BUDGET`）的一小部分，所以放大之后不会再有任何缩放。预算仍然压着越过它的图，按附件层投射请求图片所用的同一套几何，而这些图是开着平滑缩小的。
...
宿主无法指名某个浏览器，所以调用是反向走的。两个工具共用这条通道。工具体不写任何东西：它登记一次等待，并把这次调用发布到该会话自己的 `contentAccess` projection 上，而每个已连接的浏览器本来就在接收它。正在展示该会话的 page 座位在 `POST /content-frame/claim` 上认领这次调用，把活干完——遍历 frame 的文档，或者把步骤跑在它上面——再把答案 `POST /content-frame/report` 回来——两个地址都以页面自己的 base 请求，见[提供文件](#serving-the-application)。只有认领方那个标签页的回报会被接受——这也是「认领」是一次往返而不是一次通告的原因，也是同一个会话上开着的两个控制台只会跑一份步骤而不是两份的原因。两个工具的差别只在回传的那份文档，由它自己的 status 区分；开出这次等待的那个工具，才是判断手上这份文档答不答得了自己这次调用的那一方。
...
读取这一半住在 page 座位里，因为只有这个位置持有 frame 元素。可见性与几何都向元素自己的 window 询问，而不是顶层的那个——frame 的布局属于那个 frame。用户没在看的标签页照样认领：它持有同样挂着的 frame、同样活着的文档，而被另一个窗口盖住的窗口、锁上的屏幕、退到后台的标签页，三者都报 `hidden`，此时控制台就开在用户眼前。可见性改为给出价排序，而不是一票否决——标签页不在前面的座位，首次出价前先等 250ms，于是只要有一个在前面的标签页，调用就归它——而放弃过的调用，每个座位都照样遗忘，否则丢掉的那次调用在它余下的生命里就再也认领不到了。正在展示的那个会话也不是过滤器：一个控制台会为它还持有页面 frame的每一个会话作答，因为切换会话是把 frame 藏起来而不是卸载它，读取就落在那个会话上一次摆在前面的那一页上。这个控制台从未展示过页面的会话，或者 frame 已被 `cacheSize` 淘汰的会话，它一次都不出价。
...
- **隐藏标签页里的时钟归浏览器管，超过约五分钟就是一分钟醒一次** —— 用户没在看的标签页会立刻把 `setTimeout` 对齐到一秒，之后 Chrome 的 intensive throttling 再把它拉到约一分钟。真机上隐藏 8.4 分钟实测，其间控制台自己那条 WebSocket 一直连着：持有活动连接并不能让页面免于这一档。本座位花的每一次等待都是这样的定时器——出价之间的间隔、不在前面的标签页首次出价前付的那个宽限、观察页面是否安静下来的窗口、一步之内两次检查之间的轮询、问在前面那个 frame 现在在哪的轮询——所以在一个长时间隐藏的标签页里，一次读取可能撞上宿主的回报截止时间，答的是控制台认领了这次读取却没能按时作答。跑批自己的截止时间按墙上时钟算，不受影响。
- **隐藏标签页里的 canvas 可能导出一张用户从未见过的帧** —— 用 `requestAnimationFrame` 作画的页面，在标签页离开视野后就不再画，于是图读导出的是最后画下的那一帧，或者什么都没有。与下面 WebGL 那条一样，这里没有任何东西能把它与「页面本来就画成这样」区分开。
...
- **只有最近 `cacheSize` 个（会话，页面）对读得到** —— 控制台为展示中那个会话之外的会话作答，靠的是该会话缓存着、且内容栏仍在展示的最新那一页；因此在那之后被撤下或被关掉的一项，会回落到它下面仍然挂着的那一页，而被上限淘汰掉的 frame 不会回来。一页都不剩的会话——这个控制台从未展示过，或者它的 frame 全被淘汰了——它的读取改由宿主的认领窗口收场。
...
- **settings 路由假定存在 HTTP 载体** —— browser 半边以页面自己的 base 为基准请求 `/content-frame/settings`。如果某种传输提供了外壳却没有把 harness 暴露在 HTTP 上，该行会失败——与 iframe 自己那条路由的处境相同。

...
- **「只问一次」的记号是每进程的** —— 拒过一次的用户在这次会话里不会被再问，但记号存在内存里，因此重载会话或重启主机之后会再问一次。凡不是以「作出决定」收场的卡——连接断了、应答者抛了——一律不留记号，于是模型下一次尝试会再竖一张卡。

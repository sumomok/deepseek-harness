...
`src/client/access/` 把 frame 里的文档读成带编号的结构——各个区域、各个控件、每张表的形状——交给看不见它的模型。它依据的规则、以及每条规则各自舍弃了什么，记在[阅读 Agent Note](../../../.agents/notes/implemented/feature/2026-09-02-content-snapshot-engine.zh.md) 里。其中两条决定了一张控制台表格读出来是什么：
...
- **格子里页面做成可点的东西，格子会交出来。** 表格为「编辑一行」画的那个图标，往往既没有 role 也没有任何名字；当作文本读它什么都不是，因此格子把它记作 `clickable` 并给一个 ref——与这套读法在页面别处交出这类图标的口径相同。
- **字段的名字，是页面画在它前面的那些字。** 表单不把标签绑到字段上时，字段由「在同时容纳两者的最小元素之内、画在它之前、自身不印出任何一行的最后一样东西」来命名，向外最远找到字段所在的那个区域为止；这些字随后只印一次，作为字段的名字。只要有别的、读者能操作的东西横在两者之间，搜索就停下。
- **页面要求填写的字段会说出来。** 页面用 `required` 或 `aria-required` 标过的字段，以及标签前面被画了一颗星的字段，行尾都跟一个 `(required)`——那颗星是表单用样式表画出来的，并没有写进文档。这项读取由 `drawnAround` 注入（`::before` 与 `::after`），默认走计算样式。
...
- **给字段命名的那些字必须被画在一个自己的元素里** —— 与字段同处一个元素的一段裸文本不会成为名字；而当名字取自画在一段更长文字里的那几个字时，那一整段仍然印成一行，于是这几个字到达读者两次。星只在字段取名所依据的那些字上读，别处不读，因此页面自己命名过、星却画在一个它没有绑定的标签上的字段，不会被报成必填。

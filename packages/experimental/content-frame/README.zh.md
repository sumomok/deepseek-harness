...
- **有分页条的表，行数报的是本页的行数。** 页面画在表下面的那条分页条，无论中间隔着这张表的多少份副本，都归这张表；因此按列固定的表由最后一份副本之后的那条分页条分页，而画在两张不同表之间的分页条只归上面那张，画在这张表所处区域之外的分页条谁也不分。配到分页条的表读作 `e30 table 20 rows on this page × 21 cols`，并把分页条的原文带在 `pagination:` 行上，整页读取与按 ref 限定到这张表的读取都带；没配到的读作 `e30 table 20 rows × 21 cols`。分页条里什么都不解析：一共多少行、用哪种语言说，都是页面自己的事。

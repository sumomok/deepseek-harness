...
import {
  columnContextText, EMPTY_COLUMN_CONTEXT, openedPageNotice,
  type ColumnContextBounds, type ContextEntry,
...
describe('the content-column context', () => {
  it('says the column is empty, and how to put something there', () => {
    expect(columnContextText([], true, BOUNDS)).toBe(`${COLUMN} is empty. content_show puts a page there.`)
    ...
    expect(columnContextText([], false, BOUNDS)).toBe(EMPTY_COLUMN_CONTEXT)
  })
  ...
  it('lists a page whose writer the log never recorded as its kind alone', () => {
    ...
    expect(columnContextText([entry({ kind: 'page', title: 'Home', front: true })], false, BOUNDS))
  ...
  it('lists ten entries and counts the rest, rather than growing with the session', () => {
    ...
    const text = columnContextText(many, false, BOUNDS)
    expect(text.split('\n').filter(line => line.startsWith('- "'))).toHaveLength(10)
    expect(text).toContain('- … and 3 older entries not listed.')
    expect(columnContextText(many.slice(0, 11), false, BOUNDS)).toContain('- … and 1 older entry not listed.')
    expect(columnContextText(many.slice(0, 10), false, BOUNDS)).not.toContain('not listed')
  })

  it('lists and cuts to what the deployment configured, not to what the defaults are', () => {
    const narrow: ColumnContextBounds = { entries: 2, fieldChars: 8 }
    const text = columnContextText([
      entry({ kind: 'page', title: 'Weekly reports', front: true }),
      entry({ kind: 'chart', title: '黄金走势' }),
      entry({ kind: 'chart', title: 'third' }),
    ], false, narrow)
    expect(text).toBe(
      `${COLUMN} holds, newest first:\n`
      + '- "Weekly …" (page)  ← in front\n'
      + '- "黄金走势" (chart)\n'
      + '- … and 1 older entry not listed.\n'
      + 'content_show puts a page in front.',
    )
  })

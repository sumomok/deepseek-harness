...
export const CONTENT_READ_IMAGE_DESCRIPTION =
  ...
  + 'character. A very small picture is enlarged so that what is attached is not below what a model reads well '
  + '— a bitmap by a whole multiple of its own pixels, a vector rasterized large — and anything past the '
  + 'request\'s pixel budget is scaled down to it. The picture is attached and nothing is read out of it — what '
  + 'it shows is the model\'s to say. The element must be visible, must have finished loading, and must be '
  + 'drawn from this origin, because a browser lets no other origin\'s pixels be exported. Requires the '
  + 'session\'s model to accept image input.'

 ...
 * after it, which is the whole of what this package injects into a frame it
 * shares an origin with. `confirm`/`alert`/`prompt` are replaced because they
 * block the frame's event loop until something answers, and the seat is that
 * something; `window.open` is replaced because a window opened behind the
 * console is a window nobody will ever look at, so the attempt is reported
...
/**
 * Watch one frame for the length of a call, and answer its dialogs.
 *
...
export function watchPage(
  ...
  /* v8 ignore next 2 -- every document a call reaches is mounted in the seat's own frame, which has a window. */
  if (view === null) return { events: () => [], stop: () => {} }
  const address = (): string => view.location.href
  // The address the page was last reported at, so a router that announces one
  // move with several events is one line rather than three.
  let routed = address()

...
- **No responsive behavior** — the ratio is applied at every width, the session column's 180px floor being the only exception, so a narrow viewport squeezes all four columns rather than folding the session column or stacking. The shipped shell's auto-collapse breakpoint and concession chain have no counterpart here; a deployment that needs them should compose ui-layout instead.
- **Below about 540px the chat column drops under 360px** — the session floor takes 180px of a narrow frame first, even with the content column collapsed, and the shell offers no drawer or overlay in its place; on a phone-width window the fold control is the way to reclaim that width.

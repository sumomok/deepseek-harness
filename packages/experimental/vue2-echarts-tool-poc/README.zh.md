...
组合了 content 栏时，那一栏已经在整屏高度上展示本 session 的图表，于是这一行把画面交出去，只留一行紧凑文字——`<标题>：已在内容区展示。`它仍然挂载引擎，因为判定和截图属于这次调用，而栏里的座位两者都不回报；调用等待期间舞台以 `position: fixed` 按真实尺寸挪到一边，判定一到就卸载。用 fixed 而非 `display: none`，理由与已发布舞台用 `visibility` 完全相同。

这一行从 `contentSurface` 投影在不在来判断自己处于哪种情形：组合了 content 栏的地方它才发布，其余地方一概没有。两种行例外。**被取代**的行保持它自己的提示，因为无论哪种情形它都不回答任何调用、也不挂载引擎。**失败**的图表把错误行留在会话里，因为一份没画出来的文档，那一栏也没什么可展示的。
...
- **紧凑卡不是通往那一栏的链接** —— 在 content 栏旁边，会话记录里的那一行是一行文字，点它什么也选不中。两个组件分属不同的包，之间没有通道，而组件也不得自行订阅任何东西。
- **紧凑的行仍要为引擎付一次代价** —— 画面归那一栏，但这次调用的判定不归；因此每张当前图表都会在屏幕外构建一个 ECharts 实例，并在判定到来时销毁。回放一份画了几百张图的会话记录时，每一行滚进视野都要付这份代价。
...
- **report 路由假定有 HTTP 载体** —— 浏览器半边 POST 到 `/show-chart/report`——node 半边注册的那条路由，按页面的部署基址解析而来。一种提供外壳却不通过 HTTP 暴露 harness 的传输会让每次调用都停在未确认。
...
- **画出来的那张图未被组装态快照覆盖** —— 这把工具模型可见的全部内容（含工具描述与结果文本）由 [`examples/content-console`](../../../examples/content-console/README.zh.md) 泳道钉住，那条泳道真实组合了这一行，且因为没有浏览器接入，每次调用都会落到 `unverified`。浏览器把一份 option 文档画成什么样，则是跑在真实组合上的 Playwright 场景。

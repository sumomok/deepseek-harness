...
- **The report route assumes an HTTP carrier** — the browser half posts to `/show-chart/report` — the route the node half registers, resolved against the page's deployment base. A transport that serves the shell without exposing the harness over HTTP would leave every call unverified.
...
- **The painted chart is not covered by an assembled snapshot** — the tool's whole model-visible surface, description and result line included, is pinned by the [`examples/content-console`](../../../examples/content-console/README.md) lane, which composes this row for real and lets every call lapse to `unverified` because no browser is attached. What a browser paints from an option document is a Playwright scenario against a real composition.

---
description: "概念验证：通过一层薄 React 桥在 React 槽系统内渲染一个 Vue 3 组件，并记录实测的打包体积代价；面向权衡在槽里放非 React 组件的维护者。"
...
- [这个验证证明了什么](#what-the-probe-shows)
- [组合方式](#composition)
- [产物代价](#bundle-cost)
...
**运行时不变式：** 不发布伴生入口。本包不拥有任何事件流，也不拥有可变的持久数据。它唯一的关系——头部动作的注册与拆卸时的移除——是一个由本包自己的测试演练的 slot effect，而它托管的 Vue 树完全活在一个浏览器容器里。

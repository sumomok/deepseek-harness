...
import * as VuePocInvariant from '../src/invariant.ts'
...
describe('vue-ui-poc browser half', () => {
  it('declares the services it binds', () => {
    expect(inject).toEqual(['slots', 'locale'])
  })

  it('waits for the header seat to be declared before contributing', async () => {
    const ctx = new Context()
    await ctx.plugin(SlotRegistry).await()
    ctx.provide('locale', { register: () => () => {}, bind: () => () => '' } as never)
    const fiber = ctx.plugin({ inject: [...inject], apply })
    await fiber.await()
    expect(headerEntryIds(ctx)).toHaveLength(0)
    ...
    expect(headerEntryIds(ctx)).toContain('vue-ui-poc')
  })
  ...
  it('registers the header action, and fiber teardown removes it (HMR safety)', async () => {
    ...
    expect(headerEntryIds(ctx)).toContain('vue-ui-poc')
    await fiber.dispose()
    expect(headerEntryIds(ctx)).not.toContain('vue-ui-poc')
  })

  it('registers both dictionaries under its own namespace and releases them with the fiber', async () => {
    const { ctx, fiber } = await bench()
    const translate = ctx.locale.bind(NS)
    expect(translate('probe.title')).toBe(zh['probe.title'])
    ctx.locale.setLocale('en')
    expect(translate('probe.title')).toBe(en['probe.title'])

    // Withdrawn dictionaries leave the key unresolved rather than translated.
    await fiber.dispose()
    expect(translate('probe.title')).not.toBe(en['probe.title'])
  })
...
describe('vue-ui-poc invariant companion', () => {
  it('reserves package ownership under its declared companion name', async () => {
    const ctx = new Context()
    await ctx.plugin(InvariantRegistry, { enabled: true })
    const fiber = ctx.plugin(VuePocInvariant)
    await fiber.await()
    expect(VuePocInvariant.name).toBe('experimental-vue-ui-poc-invariant')
    expect(VuePocInvariant.inject).toEqual(['invariants'])

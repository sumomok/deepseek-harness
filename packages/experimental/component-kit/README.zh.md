...
- **`toy.table`**（`TableDetailRenderer`）—— 分列的多行数据，用户选了什么、打开了哪一行、按哪一列排序、按了哪个按钮，都报回去。属性：`tableConfig.gridItems`（每项 `{ relatedMetaAttr, alias?, isShow?, isSortable?, relatedComponent?, relatedComponentObj? }`）、`displayValueList`、`rawValueList`、`selectMode`（`checkbox` / `radio`）、`isNameClick`、`tableSortable`、`customOperations`（每项 `{ key, label }`）、`operationColumnWidth`。回来的动作有四个：`select` 带 `rowIndexes`、`row-click` 带 `rowIndex`、`sort` 带 `prop` 与 `order`（`asc` / `desc` / `none`）、`operation` 带 `opId` 与 `rowIndex`。
...
- **`el.metric`**（`TcProcessBallRenderer`）—— 一个数字加一行说明，画成一颗按这个数字灌满的球。属性：`process`（那个数字，同时按百分比决定水位）、`text`、`size`（直径，像素）、`background`、`borderColor`、`pointColor`、`isPointShow`。它不上报任何东西，因此动作出口与动作状态都不读。

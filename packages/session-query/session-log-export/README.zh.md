...
附件存储拿不出来的图片不属于上述失败。归档会用 `media/<attachmentId>.<ext>.error.txt` 代替那一张图片，写明日志里持有的引用——附件 id、媒体类型、字节长度、像素尺寸——以及存储抛出 `AttachmentError` 时的失败码与原因；其他种类的失败只记录失败本身，不记录它自己的文字，因为那类文字并不承诺不含主机路径。归档中其余每个文件都是完整的，下载照常成功，因此一条写错了媒体类型的引用，或一个在存储底下被删除、被改写的对象，代价是那张图片而不是整次导出。
...
- **读不出来的图片仍按记录的大小声明**——测量从不打开已存储的对象，因此一个最终由流记录下来（而非携带）的条目，仍按引用记录的字节长度计入。条目数依旧准确；两个规模字段会高出那张图片的大小，于是页面拿收到的字节去除以一个大于实际响应体的线上估算值。进度条因此走得比传输更慢，并从它停下的位置直接跳到完成——方向与「压得比标定更松的归档」相同。

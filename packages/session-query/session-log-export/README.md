...
An attachment the store cannot produce is not one of those failures. The archive carries `media/<attachmentId>.<ext>.error.txt` in place of that one image, or `files/<digest-prefix>/<digest>/<name>.error.txt` in place of that one file, naming the reference the log holds — attachment id, plus media type, byte length and pixel dimensions for an image, or stored name and byte length for a file — plus the store's failure code and reason when it raised an `AttachmentError`; a failure of any other kind is recorded without its own text, which carries no promise to omit host paths. Every other file in the archive is complete and the download succeeds, so a reference written with the wrong media type, or an object deleted or rewritten underneath the store, costs that attachment and not the export. A file the store starts producing and then fails partway through is the exception: those bytes are already on the wire, so that failure tears the archive like a torn log read.
...
- **An unreadable attachment is still announced at its recorded size** — measuring never opens a stored object, so an entry the stream turns out to record rather than carry is counted at the reference's byte length. The entry count stays exact; the two size fields read high by that attachment's size, so the page divides its received bytes by a wire estimate larger than the body it gets. The bar therefore advances more slowly than the transfer does and jumps to complete from wherever it had reached — the same direction as a generic text file, which is measured at its stored size and reaches the wire deflated.

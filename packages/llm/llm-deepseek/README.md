...
- **`ctx.attachments`** — image and file requests resolve this service at request time, so Cordis load order does not freeze optional attachment availability. Absence rejects image or file input with `UNSUPPORTED_CONTENT`; text-only calls do not require the service.
...
- A `FileBlock` never reaches the wire: `request()` lowers every file block in the (image-offloaded) request messages to a language-tagged fenced text block (`dsh-llm`'s `lowerFileBlocksFromStore`) before either serialization path runs, so `serializeRequest`/`serializeRequestWithImages` see only `text` blocks.

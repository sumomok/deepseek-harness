...
- **`GenerateOptions` 采样包含 `temperature`／`maxTokens`／`stop`，旁边还有 `responseFormat` 应答格式约束**——没有 `tool_choice`、`top_p` 或 penalty 字段；有产生方需要时才添加字段，而 `responseFormat` 的产生方是 `@haoran/dsh-llm-permission-gateway` 的判官请求，桌面载荷里自 0.4.6 起每一次审查都设置它；`packages/bundle/*` 的任何 profile 都不点名产生方，所以只由本仓库组合出来的组合仍然不会送出应答格式字段（见[已删除惰性旋钮](../../../.agents/notes/archived/simplification/2026-07-04-drop-inert-request-knobs.md)、[应答格式](../../../.agents/notes/implemented/feature/2026-09-17-llm-response-format.zh.md)）。

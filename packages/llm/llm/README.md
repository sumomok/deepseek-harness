...
- **`GenerateOptions` sampling is `temperature`/`maxTokens`/`stop`, beside the `responseFormat` answer-format constraint** — no `tool_choice`, `top_p`, or penalty fields; a field is added once a producer needs it, and `responseFormat` has no producer in the shipped composition yet — the first will be the judge request of `@haoran/dsh-llm-permission-gateway`, which sets it from 0.4.5 ([dropped inert knobs](../../../.agents/notes/archived/simplification/2026-07-04-drop-inert-request-knobs.md), [answer format](../../../.agents/notes/implemented/feature/2026-09-17-llm-response-format.md)).

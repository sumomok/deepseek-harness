...
At each eligible `agent/pre-step`, the plugin snapshots the calling session's skill catalog, applies exact `skill` tool visibility, filters to model-invocable skills, and compares a digest of the entries against the newest visible `skill-catalog` message in the session log. When the digest changed, it hands the `enter` decision a durable user-role message containing the complete replacement catalog; an empty replacement explicitly retires earlier names. An incomplete snapshot that still carries model-invocable skills publishes them — a local provider that dropped one unreadable root still reached the others — while an incomplete snapshot carrying none emits nothing and preserves the last-good view for the next pre-step, because an empty replacement would retire names that may still exist. The visibility check compares against the exact tool definition this plugin registered, so a scoped same-name shadow removes both the schema and its guidance; the plugin works mounted globally or inside one agent's composition.

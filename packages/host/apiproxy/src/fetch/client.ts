...
import {
  ...
  hostProbeTargetsValueSchema,
} from '../api/host.schema.ts'
import {
  sessionCancelValueSchema,
  sessionAttachmentValueSchema,
  sessionCreateValueSchema,
  sessionFileValueSchema,
...
export interface IApiClient {
  sessions: {
    ...
    file(payload: RequestPayload<'session.file'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'session.file'>>>
  ...
  host: {
    ...
    probeTargets(payload: RequestPayload<'host.probeTargets'>, signal?: AbortSignal): Promise<RpcResponse<ResponseValue<'host.probeTargets'>>>
  }
  workspace: {
...
const UNARY_VALUE_SCHEMAS: { [K in keyof RpcMethodMap]: z.ZodType<Wire<ResponseValue<K>>> } = {
  ...
  'session.file': sessionFileValueSchema,
  ...
  'host.probeTargets': hostProbeTargetsValueSchema,
...
export abstract class AbstractApiClient implements IApiClient {
  ...
  readonly sessions: IApiClient['sessions'] = {
    ...
    file: (payload, signal) => this.callUnary('session.file', payload, signal),
  ...
  readonly host: IApiClient['host'] = {
    ...
    probeTargets: (payload, signal) => this.callUnary('host.probeTargets', payload, signal),
  }

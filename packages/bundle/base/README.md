...
Out of the box, every profile built on this core provides: a DeepSeek model connection (the provider and model are configurable, and you can enable extra providers from your settings), the full tool set — file editing, shell commands, web search, public HTTP(S) fetch, subagents, task and goal tracking — durable sessions that survive restarts, and the default permission policy that confines file writes to your workspace and asks before risky actions. Web fetch runs without per-call approval; its provider rejects non-public destinations. Feedback stays in the Session log. Three DeepSeek-bound reporters ship off in this product: the [OTel session upload](../../session/session-telemetry-otel/README.md) row and the [installed-plugin inventory](../../llm/plugin-package-inventory-deepseek/README.md) row are declared `disabled: true`, and the [DeepSeek session-log contributor](../../session/session-log-deepseek/README.md) row carries `enabled: false`, the plugin's own switch, which returns from `apply()` before it registers the request contribution that is everything that plugin contributes. Each row keeps its upstream configuration declared, so a profile patch that re-enables one gets upstream's own behavior — `FEEDBACK_ONLY` OTel capture on feedback, ratings, edits, and withdrawals, releasing the complete canonical prefix including context.

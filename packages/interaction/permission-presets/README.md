...
User-facing permission presets through `ctx.permissionPresets` ([`PermissionPresetService`](src/index.ts)). Each configured name bundles `sandbox/mode` with `approval/policy`; the defaults are `workspace-write` (`workspace-write` + `ask`) and `danger-full-access` (`danger-full-access` + `never`). UI adapters may expose the table as one selector, while sandbox execution and approval continue to consume their own knobs. A table entry may also carry presentation: `name`, `description`, and `glyph` — which design-set glyph the selector shows, one of `read-only`, `workspace-write`, or `danger-full-access`; a preset whose id is itself a glyph name needs none.

...
| `ctx.contentSurface` | `core` | `content-surface` | - | `content-frame`, `vue2-echarts-tool-poc` | - | 拥有 extractor 表和从中折叠出的单一 `contentSurface` projection；每种内容 kind 在日志里登记自己认得什么，content-column 通过一个 keyed slot 画出被选中的那条 entry。 |

...
flowchart LR
  ...
  pkg_biz_backend --> svc_bizBackend
...
| `ctx.bizBackend` | `core` | `biz-backend` | - | `component-surface` | - | 负责对本部署自有数据后端的两次具名读取，花的是访客的访问令牌；auth-gate 用它校验过的基址和它持有的令牌构造这个服务，于是凭据留在那个包的闭包里。 |

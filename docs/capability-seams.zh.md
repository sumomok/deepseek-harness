...
| `ctx.contentSurface` | `core` | `content-surface` | - | `content-frame`, `vue2-echarts-tool-poc`, `component-surface` | - | 拥有 extractor 表和从中折叠出的单一 `contentSurface` projection；每种内容 kind 在日志里登记自己认得什么，content-column 通过一个 keyed slot 画出被选中的那条 entry。 |
| `ctx.bizBackend` | `core` | `biz-backend` | - | `component-surface` | - | 拥有对一套部署自己的数据后端的两次具名读取，花的是访客的访问令牌；auth-gate 用它校验过的基址和它持有的令牌把这个服务构造出来，于是凭据留在那个包的闭包里。 |

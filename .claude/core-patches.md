...
- **提交信息订正**：本族有一条 `fix(scripts)` 提交的信息写「`upstream/master` 只验了引用存在、没验它是不是本线的基座，引用过期会静默扩大范围……现在与 `git merge-base upstream/master HEAD` 比对」。这条修法本身是错的：`upstream/master` 落后真实基座时它仍是 HEAD 的合并基座，范围照样被撑大而门禁放行；而它前进（每次 `git fetch upstream` 之后的常态）时门禁反而失败，且提示里的「fetch」只会让它更红。基座已改由本文件的 `**基座合并**` 声明给出、在 HEAD 自己的历史里解析，门禁不再读 `upstream/master`。本线只追加提交、不改写历史，以本条为准。

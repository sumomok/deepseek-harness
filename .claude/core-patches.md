...
- **状态**：**SKIPPED — 需要设计（rc.26 同步，基座 0.1.2-alpha.2）**。三条独立障碍同时命中：(1) 依赖 `{kind:'file'}` 消息分片渲染，其上游对应物即 d56a5f7348（wire/日志/请求物化）已因 `packages/host/apiproxy` 整包搬迁而 SKIPPED，故这里既无分片可渲染；(2) 依赖 `attachment-labels.ts`／`ComposerFileAttachment`，其上游对应物即 98020a23cd（composer 草稿）已因 `sendSession` 回显系统改写而 SKIPPED；(3) 本补丁自身直接触碰 `ui-conversation/src/client/service.ts`（`sendSession` 附近再追加 53 行）与已整体搬迁到 `ui-chat` 的 `AssistantMarkdown.tsx`/`ChatNodeSeat.tsx`/`ChatView.tsx`/`MessageItem.tsx`，命中与 98020a23cd 相同的回显系统冲突外加 88be8656ab 记录的 chat 包搬迁。三者任一不解决都无法机械重落；`referent/open` 本身（ROOT 作用域可拦截打开缝隙，独立于文件附件特性）是一个值得未来单独考虑重新引入的通用能力，但本提交把它与文件卡片渲染耦合在同一个提交里，不做手术拆分，整体标记 SKIPPED，交由协调者设计端口（含是否值得先落地不依赖文件分片的 `referent/open` 本身）。已按红线原地停下、`git cherry-pick --skip`，未重落。
...
- **状态**：**SKIPPED — 需要设计（rc.26 同步，基座 0.1.2-alpha.2）**。落点 `packages/llm/llm/src/file-lowering.ts`/`tests/file-lowering.spec.ts` 正是已 SKIPPED 的 d56a5f7348 引入、又因 `packages/host/apiproxy` 整体消失而未创建的文件（该退役记录已预告"5cd83e5a14（spill）等下游补丁... 同样标记 SKIPPED"，此处兑现）。cherry-pick 冲突已确认（大量 modify/delete 与内容冲突，含 `file-lowering.ts`/`file-lowering.spec.ts`、两个 LLM 适配器、`apps/web` 快照与多份文档/生成文件），已 `git cherry-pick --skip`，未重落。交由协调者与 d56a5f7348 一并设计端口。本条目取代 87dc70d4bf 的原始 ledger 记录。
